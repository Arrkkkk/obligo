...
**Version:** v0.65 (DRAFT — not yet frozen; all 16 v0.28 proposals are ruled and adopted into live rule sections — see §20's status line and §20.4's adjudication log. **v0.29 added §6.1**, the two-run exception; **v0.30 reconciled §6.1's tie rule with `report.py`'s G2**; **v0.31 amends §8.3.1** — its v0.23 default is unreachable by the pipeline — and opens **§22**, the conforming blocker, **decided at v0.32 as a deliberate deferral**. **v0.33 puts §9's dual denominator IN FORCE** on two independent grounds, adds §5 clause 5's number rule, §3.4's bounded freeze-pass exception, and the forward authoring rules §3.6 / §3.8.2 / §3.5.1 — **no annotation rule changed, no item restamped, no cassette stale**. **v0.34 records two MEASUREMENT corrections and changes no rule either** — §8.9's `on`/`until` rows and §15.3's loud-versus-silent class placement, both falsified by real model output from the compile-stage bottleneck investigation. **v0.35 adds §4.3.2** — a third splitting shape distinct from both existing §4.3 worked examples, reviewer-ruled at batch 3's `C04-139`: self-performance vs. a duty to bind/control a third party's conduct splits into two items even when the two verbs share an object phrase and a sentence subject, because the two performances do not share an actor. **v0.36 amends §4.2** — the resulting byte-identical spans (`C04-139`'s shared trailing object leaves neither item's minimal span shorter than the other's) break IoU's ability to discriminate between the two gold items, so a content-based tie-break on `action_accept_set` membership is added, falling through to ascending `item_id` when the tie-break itself is inconclusive. **v0.37 adds a **v0.61 ADDS NO ITEM, the set stays at 36, and NO PUBLISHED FIGURE MOVES: §10.1 F19 is ruled (§8.11), `E03-01` gains a SECOND known gap `lead_time_unrepresentable` (`REPRESENTATIONAL` / `INCOMPLETENESS`), and in-force criterion 2 holds at `3/10 = 30.0%`, all-items at `5/15 = 33.3%`, `K` at `14/32` — confirmed by a real `run_scoring.run()` before and after.** The annotator's *conclusion* survives (the temporal fails even unredacted — both the phrase as emitted on 3 of 3 runs and a plausible unredacted one return `UNMAPPABLE_TEMPORAL` through the production `compile_candidate()`); the *mechanism* does not. **It is NOT a §7 composition gap**: no v1 form pairs a `duration` with a `direction`, so granting free composition of all five forms would not rescue the clause — the gap is **single-form expressiveness**, and the nearest composable approximation (`WITHIN 30d OF X` ∧ `BEFORE X`) denotes the **complement** of *"at least 30 days before X"*. **NOT RESTAMPED, with the alternative MEASURED rather than argued**: restamping would stale `E03-005`'s cassettes and move all-items `5/15 = 33.3%` → `5/14 = 35.7%` purely by dropping a `PARTIAL` item out of the denominator. **THE TWO ANNOTATORS' AGREEMENT ON `redacted_value` IS NOT CORROBORATION** — cold explicitly declined the composed reading and named a *different* composed element, so set equality on `known_gaps` is agreement about a tag and never about the mechanism behind it. **§8's `EVERY`+`DURING` ROW IS CORRECTED**: it stated the grammar path where the extraction path is `UNMAPPABLE_TEMPORAL` — the **fourth** tracked instance of a grammar-layer guarantee the extraction path bypasses, and the first that had **propagated**, having been cited by an annotator as a reason not to tag. **F17 is UNMOVED** (kind-scoped band holds at `(17, 19)`; `redacted_value` already excluded the item on both sides) **but `G` moves `6/31 → 7/31`** — and, on the CONFORMING-scoped `G`, `4/26 → 5/26`, which **crosses that instrument's `REDESIGN` anchor**, so F19 is **not** verdict-neutral and this ruling's own first pricing said it was (corrected before landing; `G_swing` and the `(15,15)` scoreability point are unaffected). Pricing it surfaced a real defect in `G`'s reproduction, **closed here**: it read `known_gaps` live against a transcribed population, four items have drifted since the seal, and the published `G = 6/31` had been surviving by **coincidence** — two drifts cancelling — while `G_swing` and the band had already moved. Population is now derived from the sealed `comparison.json` and a `PRE_TAG` map restores a genuine historical reproduction, mirroring K's own v0.53 fix. **One new entry opened rather than folded in, F20** (should a future §7 run seal `known_gaps` into its artifact, so override maps become unnecessary?). **v0.62 EXECUTES §10.1 F1 — THE FIRST QUEUE ENTRY SINCE F9 TO MOVE A PUBLISHED FIGURE, AND THE FIRST SINCE THE FREEZE-PASS QUEUE OPENED TO MOVE ONE *UPWARD*: in-force criterion 2 `3/10 = 30.0%` → `5/10 = 50.0%`, all-items `5/15` → `7/15 = 46.7%`, confirmed by a real `run_scoring.run()` before and after.** All three queued accept-set widenings land as ONE atomic event, the sequencing F5 made binding: `C02-01` gains *"Antares or its Subcontractor"*, `C02-03` gains `invoice_costs`, `C11-01` gains `principal_interest`. **NO ITEM ADDED (the set stays at 36), NO ITEM RESTAMPED, NO CASSETTE STALED, NO MEMBER REMOVED** — and scoreability is unchanged at 15 scored / 21 cassette-unscoreable, so nothing is bought by dropping an item. **The v0.58 sizing was RECONFIRMED, not carried forward** — it was stated on a `/9` denominator v0.59's kind-scoping has since moved to `/10` — and the `+2` reproduces exactly, movers `C02-01` and `C02-03`. **`C11-01` is worth ZERO as predicted** (modal stays `PARTIAL`; one of three failing clauses removed). **The widen-and-restamp alternative was MEASURED and refused at `3/7 = 42.9%`** — both movers leave the denominator, the numerator does not move, and the figure improves purely by shrinking. **A second-instrument effect the sizing never priced is disclosed rather than found later (§3.6.1's dual-predicate rule): `K` unmoved at `14/32`, in-force `A` unmoved at `7/27`, but the published LEGACY per-clause profile would have drifted `5_object` 6 → 5 via `C11-01`** — held by the `PRE_RESTAMP` mechanism F11/F13 already use, so the dated run still reproduces exactly. **A LATENT DEFECT IN THAT MECHANISM IS FIXED: `PRE_RESTAMP` held `C10-01` twice, so F19's v0.61 entry silently overwrote F11's v0.44 slot correction** — inert (the item fails `5_object` either way, so no published figure was ever wrong) but the guard was dead for two rulings; merged, with an AST duplicate-key test verified RED against the reintroduced defect. **v0.64 ADDS NO ITEM — the set stays at 48 — AND NO PUBLISHED FIGURE MOVES: §10.1 F22 is ruled at §8.8.5. §8.8's SUB-CHOICE (single-verb `action_accept_set`, no widening) is CONFIRMED and its *"v0.24 default, pending confirmation"* flag is DISCHARGED; in-force criterion 2 holds at `5/10 = 50.0%`, all-items at `7/15 = 46.7%`, `K` at `14/32`, `A` at `7/27` — confirmed by a real `run_scoring.run()` before and after, not hand-computed.** **THE §8.8-vs-§14.3 TENSION IS A ROUTING ERROR AT §14.1, NOT A CLASH OF TWO LIVE RULES**: §14.3's precondition is *sentence* ambiguity (*"`AMBIGUOUS` is a property of the text"*), a genuine gap is *encoding* uncertainty — §14.1's `UNCERTAIN`, routed to §14.4 — and **all three logged items in fact took §14.4**, every one carrying `adjudicated_by: reviewer`. **A second ground is stated because `C10-02`'s case is genuinely arguable and saying so is cheaper than defeating it**: §14.3's remedy presupposes each competing reading is REPRESENTABLE, so a widening here records indecision among known-wrong answers, which is §14.2's prohibition applied to clause 2 instead of clause 8. The principle is promoted from §14.2 rather than authored — **`action_accept_set` records what the SENTENCE admits, never what the TAXONOMY forces the annotator to settle for** — in the shape §8.6.1 took. **THE NARROW FIX IS REJECTED ON A MEASUREMENT, NOT A PREFERENCE: across 4 of 4 items that recorded a runner-up it admits NOTHING** — the runner-up is always a *competitor* to the nearest verb (`GRANT` against `ESTABLISH`, `DELIVER` against `PROCESS`) rather than a synonym of it, and where genuine synonyms exist (`E03-03`'s `GUARANTEE`/`WARRANT`/`ASSURE`) they are OFF-TAXONOMY, which is what made the item a gap; its second reading is §8.8.2's mappability test, which returns NO for the whole class by construction; and it has no mechanical test, §8.3.2's own sustained objection. **THE EMPTY-SET ALTERNATIVE IS RE-PRICED AGAINST THIS RULING'S OWN CONCLUSION and still rejected**: v0.59's kind-scoping has since removed every item in the class from §9.1's in-force denominator, so §8.8's decisive v0.24 cost (*"a guaranteed failure for roughly one item in four"*) now lands only on the all-items figure — it is far cheaper than when deferred, and is refused on the narrower ground that both figures are recoverable from the committed data either way. **THE CLASS IS EIGHT ITEMS, NOT THREE** (16.7% of 48, inside §8.8's sampled 23% CI [11%, 42%], which is deliberately NOT restated per §8.8.2's own rule) — all eight already carry a singleton, so the three logged instances are only where the collision was noticed. **`E08-04`'s PROVISIONAL DISPOSITION IS DISCHARGED, NOT CHANGED**: not one field moves and it is NOT restamped (`C11-02`/`C04-06`'s v0.54 precedent — a rule that confirms rather than changes earns no restamp, and the fact that its segment has no cassette makes it *free*, which is not a reason). **A CONFIRMATION MUST NOT BANK THE SECTION'S OWN COMPLAINT**, so a reporting rule is added: clause 2 on an `action_not_in_taxonomy` item is disclosed wherever criterion-2 figures appear as a **forced-choice agreement check, not an action-identification measurement** (`report.py` G9) — the honest half of the empty-set alternative without its data change. **COST IS MEASURED ON FOUR INDEPENDENT GROUNDS, each verified by execution, and the load-bearing half is that a WIDENING COULD NOT HAVE MOVED A FIGURE EITHER** — so the ruling could not have been reached for to protect a number: the tag is `REPRESENTATIONAL` so §9.1 excludes it; all eight items are cassette-unscoreable today; `C10-02` and `C14-04` are `NON_CONFORMING` under §5.1's A1 gate so they sit outside both sides of `A` and `G`; and **no LEGAL widening can flip the legacy published `K`'s `2_action`**, since cold's paired slots are off-taxonomy and §3.4 admits only taxonomy verbs into a gold accept-set. **A first draft of the cassette row was WRONG** — `E01-01` and `C04-06` *are* cassette-backed — corrected by reading the run's own G8 list rather than the cassette directory. **`annotator_confidence` is deliberately NOT corrected on the four `AMBIGUOUS` items**: the field is read by no code under `apps/brain` (verified) and `C10-02`'s reading is arguable.
...
**v0.63 ADDS NO ITEM — the set stays at 48 — AND NO PUBLISHED FIGURE MOVES: §10.1's two logged `temporal-composition` instances are ruled at §8.12. A new tag `temporal_composition` is added (kind `REPRESENTATIONAL`), three items gain it (`E03-01`, `C02-07`, `C04-08`), and in-force criterion 2 holds at `5/10 = 50.0%`, all-items at `7/15 = 46.7%`, `K` at `14/32` — confirmed by a real `run_scoring.run()` before and after, not hand-computed.** The gap is a **SLOT** gap, not a form gap: each element is separately classifiable but `ast.Obligation` holds ONE `temporal`, so the sibling is `compound_action` (§8.3) and not §8.11's single-form `lead_time_unrepresentable`. **BOTH LOGGED NOTES ARE CORRECTED ON THEIR CENTRAL CLAIM** — *"each element has a v1 form on its own"* is false as written (one of three for instance one, two of three for instance two; `during the Term` and `until <event>` both classify to `None`), so instance one is a **mixed** case and the §8.11 distinction survives on instance two alone. **THE SCOPE IS THE SLOT LIMIT, NOT THE GREEDY `(.+)$` SWALLOW**, which is a symptom confined to surplus sitting inside the quoted phrase — so `C02-047`'s logged near-miss is IN scope and `C02-07` is tagged, while the instance count behind the amendment stays at **two** (what counts as an INSTANCE is not what falls in the TAG'S SCOPE), and `E03-01` is swept in from §8.11's own *"the note omits `during the Term` entirely"*. **DIRECTION IS DEFERRED BY RULING, AND THE NOTES' OWN GROUND FOR DEFERRING IT IS THE WRONG ONE**: they reason from the pipeline's swallowed trigger string where §8.10.1's test is whether GOLD departs from the document, and gold never swallows. The real ground is that direction **varies by instance and within one item** — a dropped narrowing bound is OVERSTATING-shaped, a dropped additive deadline INCOMPLETENESS-shaped, and `C04-08` drops one of each. **An OVERSTATING assignment was drafted and REJECTED** despite being attested 2 of 2. So axis 2 gains a third state, `DIRECTION_DEFERRED`, spelled explicitly rather than as a missing map entry, and **F21 is opened** to ask whether direction belongs to the tag or to the item. **Exclusion-neutral** — all three items were already outside §9.1's in-force denominator — **no item restamped, no cassette staled**. **A SIXTH INSTANCE of §10's close-out drift is corrected in passing**: the Status line read *"36 items locked"* after batch 4 locked the set at 48, batch 4 having touched no guideline file.

**v0.65 ADDS TWO ITEMS — the set moves 48 → 50 — AND NO PUBLISHED FIGURE MOVES: in-force criterion 2 holds at `5/10 = 50.0%`, all-items at `7/15 = 46.7%`, `K` at `14/32` and `A` at `7/27`, confirmed by a real `run_scoring.run()` BEFORE AND AFTER rather than hand-computed.** Batch 5 opens at seed `20260919`'s hard queue position 6 (`E02-006`) and yields `E02-01` and `E02-02`, the gold set's first items from document E02. **THREE THINGS ARE RULED OR CORRECTED, and two of them are corrections to claims this document has carried for a long time.** **(1) §8.13 MINTS `within_parenthetical`, kind `REACHABILITY`, no direction, excluded from §9.1's in-force denominator on the structurally-guaranteed-compile-failure ground — identical treatment to `within_preposition`.** §11 decision 1 has named that tag in the imperative since **v0.8** and it was a **PAPER TAG**: absent from `gap_kinds.GAP_KIND`, absent from `annotator_agreement.SECTION_8_TAGS`, carried by zero of 48 items, so §8.10's forcing function would have failed the first item that used it. **§11's instruction and §8.6's "two gaps, one tag" practice CONTRADICTED each other — the same shape as F13's original conflict — and are RECONCILED explicitly rather than silently merged**: both survive, because a phrase files the tag for its own least-repairable obstacle and exactly one tag, so `C11-03` keeps `within_preposition` alone and **is not restamped**. §10 re-check logged and gated on a known answer: of four locked spans carrying a `WITHIN`, exposure is **one item, the new one** — `C11-02` is a screen false positive **adjudicated rather than reported** (its phrase sits in its `if`-clause and its own `temporal` is `null`), which matters because it is one of the ten items in the in-force denominator. **(2) THE CALENDAR-DAYS UNIT GAP IS DELIBERATELY NOT MINTED (§10.1 F23)**, on a measurement: 8 instances across 6 documents and **ZERO occur without the numeral gap**, so a separate tag could never fire alone — §22.1's a-kind-is-not-ruled-on-a-sample-of-one, and a test pins its absence from both vocabularies. The 179-deadline decomposition behind both rulings is preserved at `holdout/within_parenthetical/screen.py`, double-gated (six known answers **and** exact reconciliation with `corpus.py`'s own `20/159`); **its first draft failed that second gate in the instructive direction, returning a clean ZERO for the unit class by reproducing `corpus.py`'s conflation instead of measuring it**, and that failure is kept in the file. A related defect is **reported and not fixed**: `corpus.py`'s `classify_within()` inflates the published `within_parenthetical` statistic by 8/159 (5.0%), and §11's open widening decision is made against that number. **(3) §21 R1's CITED EVIDENCE IS CORRECTED IN PLACE — the rule stands, the measurement does not.** All three collisions it names (`Client` in E02/E07; `Provider`/`Recipient` in C17/E01) were checked and **none reproduces**: E02 defines no bare `Client` (only `Client Facility`, an unnamed third party R3 excludes) and E01 defines `Service Provider`/`Service Recipient`, never the bare terms. Its stated consequence was tested by execution — a merged registry resolves all three aliases **uniquely**, so `E07-01` and `C17-01` would not flip. Per-document registries remain correct on the surviving near-miss ground; the same correction is applied to `registry.py`'s duplicate docstring claim and to CLAUDE.md. **`registry/E02.json` is authored to §21 R1–R3** (the two preamble parties; `Client Facility` deliberately excluded), which is what lets `E02-01`'s `underspecified: false` be **proven against a committed fixture rather than asserted** — the first item in the set where a missing registry was load-bearing on a scored clause. **No item restamped, no cassette staled, no annotation rule changed.**

**Status (v0.65): 50 items locked (batch 5 opens with E02-01/E02-02); §8.13 ruled — `within_parenthetical` MINTED (`REACHABILITY`, no direction, excludes), §11's paper tag finally implemented and reconciled with §8.6's two-gaps-one-tag practice; the calendar-days unit gap NOT minted (§10.1 F23); §21 R1's cited evidence corrected in place, the rule unchanged; `registry/E02.json` authored. In-force criterion 2 `5/10 = 50.0%`, all-items `7/15 = 46.7%`, `K` `14/32`, `A` `7/27` — no item restamped, no cassette staled.** *(Prior text follows, left as written per the corrections-are-new-text discipline.)*

**Status (v0.64): 48 items locked; §8.8.5 ruled (§10.1 F22) — §8.8's single-verb accept-set CONFIRMED and its *"pending confirmation"* flag discharged; in-force criterion 2 is `5/10 = 50.0%`, all-items `7/15 = 46.7%`, `K` `14/32`, `A` `7/27`; no tag added, no item added, restamped or re-authored, no cassette staled, one reporting disclosure added (G9).** *(Prior text follows, left as written per the corrections-are-new-text discipline.)*

**Status (v0.63): 48 items locked (batch 4 added 12); §8.12 ruled — in-force criterion 2 is `5/10 = 50.0%`, all-items `7/15 = 46.7%`, `K` `14/32`, `A` `7/27`; one tag added, three items tagged, no item added or restamped, no cassette staled.** *(Prior text follows, left as written per the corrections-are-new-text discipline. The line below read "36 items locked" from v0.62 until 2026-09-23 — true when written, false the moment batch 4 locked 12 more items in a commit that touched no guideline file. Counting §19.3's running tally rather than incrementing the last claim: the header's own "no items have been annotated" (1), the v0.15/v0.17 changelog pair (2), v0.28's header/changelog (3), v0.29's Status line (4), v0.56's Status line (5) — this is the **sixth**.)*

**Status (v0.62): 36 items locked; F1 EXECUTED — in-force criterion 2 is `5/10 = 50.0%`, all-items `7/15 = 46.7%`, `K` `14/32`, `A` `7/27`; no item added or restamped, no cassette staled.** *(Prior text follows, left as written per the corrections-are-new-text discipline.)* **32 items locked; ONE restamped at v0.52** (`C17-02`, §8.6.1/§10.1 F13 — `temporal` `null` → the `WITHIN` form; `C17-066`'s three cassettes are consequently **STALE**, re-record **deferred to §10's freeze pass** by reviewer ruling, so the item is known-stale and honestly labelled, the same footing §22.1 gives `C14-02`). **`C11-01`'s `annotator_notes` were corrected record-only at v0.52 with its stamp deliberately UNCHANGED** — the note contradicted the item's own §15.5-correct data, and restamping it would have staled three cassettes for nothing. ~~**TWO SPANS ARE NOW `PENDING_ANNOTATION` and are NOT yet items** — `C11-094` #3 and `C04-117` #1, ruled eligible by §10.2 (Decision 3); each owes its own §14.4 drafting session, with §8.8's taxonomy question flagged in its segment file.~~ **SUPERSEDED v0.53 — BOTH SPANS ARE NOW DRAFTED ITEMS, so the set stands at 34, not 32.** **SUPERSEDED AGAIN, v0.56 — THE SET STANDS AT 35.** `C11-03` was ruled at v0.55 (`C11-094`'s last escalated span) and this line was not updated then, which is the fourth logged instance of exactly the drift §10's own close-out process note exists to catch — corrected in place here, per that note's rule that live status statements are corrected rather than struck. **v0.56 adds no item**: §8.8.4 rules `C14-076` candidate 1 `NOT_OBLIGATION_BEARING`, so the set stays at 35 and no published figure moves. **SUPERSEDED v0.57 — THE SET STANDS AT 36.** `C14-076`'s band escalation is ruled: candidate 2 yields one item, `C14-06`, from verb 2 only; the segment reconciles at 3 clauses, at the band ceiling and eligible, and `C14-01`/`C14-02` stay locked. Still no published figure moves — `C14-06` is cassette-unscoreable at `v0.57`. **v0.58 ADDS NO ITEM and the set stays at 36** — §10.1 **F5** and **F12** are ruled together: §3.6.1's scope extends to the whole `action_accept_set` (§3.6.2, retroactive, widening-only, **zero cost — 35 of 36 items already conform**), `C22-01` is adjudicated explicitly under carve-out 1 with its cognate-object co-variance **accepted as irreducible**, and the breadth audit is **REDUCED to F1's three items** plus a forward head-only rule for batch 4 onward. **No item restamped, no cassette staled, no member removed, no published figure moved** — `3/9 = 33.3%`, `5/15`, `K` `14/32`, confirmed by a real run before and after. **F1's own value is now MEASURED rather than hand-computed — `3/9 → 5/9 = 55.6%`, movers `C02-01` and `C02-03`, with `C11-01` worth ZERO** — and its execution stays queued as one atomic freeze-pass event. **v0.59 ADDS NO ITEM and the set stays at 36, BUT — unlike v0.54 through v0.58 — IT MOVES A PUBLISHED FIGURE, and DOWNWARD: §10.1 F9 is ruled, §8 gains a `kind` axis (§8.10), and §9.1's in-force denominator is kind-scoped, so criterion 2 reads `3/10 = 30.0%`, NOT `3/9 = 33.3%`.** Whole set 17/36 → 20/36; movers `C04-01`, `C04-03` (`corpus_artifact_in_span`) and `C14-06` (`shared_subject_split`), of which only `C04-03` is cassette-scoreable today. **The numerator does not move, so the ruling can only lower the figure.** All-items `5/15` and `K` `14/32` are untouched; **no item added, restamped or re-authored, no cassette staled, no annotation rule changed** — measured by a real `run_scoring.run()` before and after. **F10 is resolved under the same axis** (its five tags carry no direction because gold is faithful for their kinds, not because a direction was finally assigned), and **two new entries are opened rather than hidden: F17** (the G7 band is still legacy-scoped, and kind-aware it moves `(15,15)` → `(16,17)` on the conforming pairs — the point estimate v0.48 called "the substantive result" stops being a point) **and F18** (`redacted_value`'s contested direction, which is why `WITHHELD_VALUE` is held excluded; admitting it would read `3/11 = 27.3%`). `C11-02` and `C04-06` — **both REVIEWER-APPROVED at v0.54** (`adjudicated_by: reviewer`; eligibility was `RULED_BY_REVIEWER` at v0.52, the field-level drafting approved as drafted, with the singleton accept-set kept, the sibling `object_class` divergence accepted as temporary, and `conditions: []` confirmed and generalised into §3.8). **Both keep `guideline_version: v0.53`** — the v0.54 rule confirms rather than changes their treatment, so restamping is refused per §22.1. §8.8's flagged taxonomy question is **ruled in §8.8.3, with the two verbs landing on opposite sides**: `purchase` → `PROCURE` is defensibly mappable (`C11-02` carries **no** tag and `known_gaps: []`), `defer` is a genuine gap (`C04-06` carries `action_not_in_taxonomy`). **v0.60 ADDS NO ITEM, the set stays at 36, and — unlike v0.59 — NO PUBLISHED FIGURE MOVES: §10.1 F18 is ruled (§8.10.1), `WITHHELD_VALUE`'s direction is `INCOMPLETENESS`, and the kind stays EXCLUDED — so criterion 2 holds at `3/10 = 30.0%`, all-items `5/15`, `K` `14/32`, confirmed by a real run before and after.** The kind is now excluded **by ruling** rather than **held** pending one; `DENOMINATOR_EXCLUDING_KINDS` is unchanged in content. **F18's filed premise is falsified by measurement** — the redaction removes the VALUE and leaves the CONSTRUCTION, which the pipeline reads (`E03-005`, 3 of 3 runs) — so gold's `null` discards a stated constraint rather than recording an absent one, and §8.1's own recoverability channel is invisible to §5's predicate. **AGAINST INTEREST: on this item the model's IR is MORE faithful than gold's**, which is an argument for revisiting §8.1's `null` rule and deliberately NOT for admitting the item. The direction axis is restated on the test that does the work — *direction is assignable wherever gold departs from the document* — and **one new entry is opened rather than folded in, F19** (does `E03-01` also owe an untagged `REPRESENTATIONAL` composition gap, masked by its redaction tag? exclusion-neutral, unverified). **v0.61 ADDS NO ITEM, the set stays at 36, and NO PUBLISHED FIGURE MOVES: F19 is RULED (§8.11) — `E03-01`'s notes are RIGHT that its temporal fails even unredacted and WRONG that it is a §7 composition gap.** `lead_time_unrepresentable` is added (`REPRESENTATIONAL` / `INCOMPLETENESS`): no v1 form pairs a `duration` with a `direction`, so it is a **single-form** expressiveness gap that free composition would not rescue. In-force criterion 2 holds at `3/10 = 30.0%`, all-items `5/15`, `K` `14/32` — confirmed by a real run before and after; the item is **not restamped**, and the alternative was measured at all-items `5/14 = 35.7%`, a figure improving for no improvement. §8's `EVERY`+`DURING` row is corrected (it stated the grammar path; the extraction path is `UNMAPPABLE_TEMPORAL`) as the **fourth** tracked grammar-bypass instance. **F17 is unmoved (band `(17, 19)`) but `G` moves `6/31 → 7/31`, and the conforming-scoped `G` crosses its `REDESIGN` anchor (`4/26 → 5/26`) — so F19 is NOT verdict-neutral, correcting this ruling's own first pricing**. `G`'s reproduction — which read tags live and had been matching its published figure by coincidence — is fixed; **F20 is opened** for whether a future §7 run should seal `known_gaps` into its artifact. **Both are cassette-unscoreable today** — stamped `v0.53` against `v0.28`/`v0.48` cassettes, disclosed by G8, backdating rejected per §22.1 — so **no published figure moves: in-force criterion 2 stays `3/9 = 33.3%`, all-items `5/15`, `K` `14/32`**, confirmed by a real run. `C11-02` queues the in-force denominator to go **9 → 10** at §10's freeze-pass re-record (§9.2.1). **§10.1 F16 RESOLVED 2026-09-16 (commit `94a2daa`, CI run 35087036051 green) — `run_scoring.run()` now executes end to end against the committed gold set; see the F16 table row for the fix and the real-run confirmation.** *(Superseded text, left as written: "BLOCKING: `run_scoring.run()` cannot execute against the committed gold set at any single `guideline_version` (§10.1 F16), so no figure below can currently be confirmed by a run.")* — no item restamped at v0.45 (§5.1 governs annotator comparison only and touches no annotation rule) — batch 1 complete (10), batch 2 at 8 of 10 with two
...
**v0.54 change: BOTH v0.53 ITEMS ARE REVIEWER-APPROVED, and the one flagged call that
generalises is written into a rule.** `C11-02` and `C04-06` move `PENDING_REVIEW` → **APPROVED**
(`adjudicated_by: reviewer`), settling the drafter's three flagged calls explicitly rather than by
silence: the **singleton `action_accept_set` is KEPT** (its cost — an in-force denominator item
near-certain to fail clause 2 — is accepted rather than cured by a widening made *after* the
prediction was known, which §3.4's justification test forbids; the freeze pass stays the venue);
the **sibling `object_class` divergence is accepted as TEMPORARY** (`C11-01` `franchise_interest`
vs `C11-02` `principal_interest`, converging when §10.1 F1's queued widening lands); and
**`conditions: []` is CONFIRMED and generalised**.

**The new rule, at §3.8 with a cross-reference at §2.5.1: a reviewable-standard phrase is NOT a
`conditions` entry.** *"acting reasonably"* — and by the same logic *"in good faith"*, *"to its
reasonable satisfaction"* — governs **how** a right is exercised, not **whether** the obligation
applies, and `conditions` holds only the circumstances the obligation depends on. `C04-06` is the
first instance: no locked item had reached the question. **The apparent tension is the point, not
a defect** — the same phrase decides eligibility under §2.5.1 (it is that section's own
discriminator against *"sole business judgment"*) and is excluded from `conditions` under §3.8,
because the two sections ask different questions.

**NOTHING IS RESTAMPED AND NO FIGURE MOVES.** Both items keep `guideline_version: v0.53`
deliberately — the v0.54 rule **CONFIRMS** their existing treatment rather than changing it, and
restamping a confirmed item is the gratuitous conforming event §22.1 and `C11-01`'s own
record-only correction both refuse. `reviewer_status`/`adjudicated_by` are provenance and are read
...
> **RULED v0.60 — §8.10.1. The paragraph above is a dated record from here on.** Its exclusion
> **stands** and the figure does not move (`3/10 = 30.0%`), but two of its claims do not survive
> checking. *"The value is absent from the world the contract describes"* is wrong about what the
> gap is: the redaction removes the **value** and leaves the **construction**, and the pipeline
> reads the construction — measured, `E03-005` quotes the redacted timing phrase on 3 of 3 runs and
> the repaired narrowing classifies as `BEFORE "the ** of each Calendar Quarter"`. And the
> direction is **not** contested once §5's predicate is read rather than the item: it is
> `INCOMPLETENESS`, because §8.1's own recoverability channel (`missing_fields`,
> `redacted_phrase`) is invisible to the predicate that certifies `FULLY_CORRECT`. Left as written
> per this document's corrections-are-new-text discipline.
...
**v0.60 change: §10.1 F18 RULED — `WITHHELD_VALUE`'s direction is `INCOMPLETENESS` and the kind
stays EXCLUDED from §9.1's in-force denominator (§8.10.1). NO ITEM IS ADDED, RESTAMPED OR
RE-AUTHORED; NO CASSETTE IS STALED; NO ANNOTATION RULE CHANGES; AND — unlike v0.59 — NO PUBLISHED
FIGURE MOVES: in-force criterion 2 stays `3/10 = 30.0%`, all-items `5/15`, `K` `14/32`, confirmed
by a real `run_scoring.run()` before and after rather than hand-computed.** The set stands at
**36**. What changes is that the kind is excluded **by ruling** rather than **held** pending one,
and `DENOMINATOR_EXCLUDING_KINDS` is unchanged in content.

**(1) THE FILED PREMISE IS FALSIFIED BY MEASUREMENT, AND THAT IS WHAT DECIDES IT.** F18 asked
whether gold's `null` *"faithfully records that the value is genuinely absent from any party's
knowledge."* The redaction removes the **value**; it leaves the **construction** standing, and the
pipeline reads the construction. On `E03-005`, **3 of 3** runs, the model quotes
`"At least ** before the ** of each Calendar Quarter during the Term of this Supply Agreement"`
into `temporal_raw`; `_classify_temporal` returns `None`, repair fires, and the narrowing
`"before the ** of each Calendar Quarter"` **classifies** as `BEFORE "the ** of each Calendar
Quarter"` — verified against the production classifier, not read off the repair text. So gold's
`null` does not record an absent constraint; it **discards a stated one**.

**(2) GROUND 2 BITES BECAUSE §5 READS THE PREDICATE, NOT THE ITEM.** §8.1 already concedes the
hazard — *"that conflates 'the contract states no deadline' with 'a deadline exists and was
withheld from the filing'"* — and answers it with `missing_fields` + `redacted_phrase`, *"the
channel that keeps the withheld-versus-absent distinction recoverable."* §5 excludes
`missing_fields` from the predicate and `redacted_phrase` is non-scored, so **that channel is
invisible at the point of certification** and the bare `null` is what a `FULLY_CORRECT` verdict
would certify. **Recoverability from the ITEM is not recoverability from the PREDICATE.**

**(3) A SECOND, WEAKER GROUND, RECORDED SEPARATELY BECAUSE THE TWO FAIL DIFFERENTLY.** The item is
not winnable in practice — `E03-01` scores `PARTIAL` on `failed=['temporal', 'underspecified']`,
with clause 8 purely downstream of the unresolvable trigger, one root cause for both. **Stated
precisely rather than overclaimed: 3-of-3 measured model behaviour, NOT `REACHABILITY`'s
grammatical guarantee.** Ground 2 alone is sufficient.

**(4) AGAINST INTEREST, AND IT IS THE SHARPEST THING THE RULING FOUND: on this item the model's IR
is MORE faithful to the document than gold's is.** It recovers the real structure (`before
<trigger>`) and loses only the magnitude, which is genuinely unknowable from the filing; gold
discards the constraint entirely. **That is an argument for revisiting §8.1's `null` rule — not an
argument for admitting the item to the headline denominator**, which is what F18 asked, and the two
are deliberately not run together.

**(5) THE KIND IS NOT COLLAPSED INTO `REPRESENTATIONAL`, and the direction axis is restated on the
test that actually does the work.** §8.10's separating ground is untouched: **no IR change helps** —
v1 *has* the form, the classifier produced it. So two of five kinds carry a direction, which is
coherent under **direction is assignable wherever GOLD DEPARTS FROM THE DOCUMENT**. v0.59's
*"direction only for `REPRESENTATIONAL`"* was a **proxy** for that test — correct on the tags then
live, wrong in general — and is now stated as the test itself
(`gap_kinds.DIRECTION_BEARING_KINDS`).

**(6) A CONSEQUENCE FOUND BY EXECUTION, AND A FALSE JUSTIFICATION CAUGHT BEFORE IT SHIPPED.** Once
`redacted_value` carries a direction, G6's `WITHHELD_VALUE` bucket becomes **unreachable** — for
any direction-bearing kind `direction_of()` returns a direction or `UNCLASSIFIED` and never `None`,
so the `else kind_of(tag)` branch cannot fire. **The first draft of this ruling kept the bucket with
a written justification that was simply false** (that a future unruled withheld-value tag would land
in it; it routes to `UNCLASSIFIED`). Caught by checking the claim against the real functions rather
than by re-reading the code — Standing Principle 7's discipline applied to a piece of prose. The
bucket is removed, a test pins the unreachability, and the routing is the correct one.

**(7) ONE NEW QUEUE ENTRY OPENED RATHER THAN FOLDED IN — F19.** `E03-01`'s own notes record that its
temporal *"would fail even unredacted"* as an `EVERY` ∘ `RELATIVE_TO_TRIGGER` composition IR v1
cannot represent, which would mean the item carries an **untagged `REPRESENTATIONAL` gap masked by
its redaction tag**. It is exclusion-neutral today and **unverified**, and ruling a KIND on a sample
of one is the error §22.1 refused — so it is filed with its evidence, not folded into this ruling.
...
**Where the reviewable-standard phrase goes once the clause IS eligible (v0.54 — REVIEWER-RULED,
first instance: `C04-06`).** This section's discriminator does **eligibility** work and stops
there: a phrase like *"acting reasonably"* is **not** carried into `conditions`. **§3.8 holds
the rule**; the split is that this section asks whether the clause can ever be checked, while
`conditions` holds the circumstances the obligation depends on. `C04-06`'s *"Miltenyi, **acting
reasonably**, reserves the right to defer…"* clears this section **on** that phrase and is
annotated `conditions: []`. Cross-referenced here so a future annotator reaching this section
does not infer that a phrase load-bearing for eligibility must also be a condition entry.
...
**EXECUTED v0.62 — F1 HAS LANDED, AS ONE ATOMIC EVENT, AND THE PARAGRAPH ABOVE IS A DATED
RECORD.** All three widenings are in the committed gold set: `C02-01`'s `obligor_accept_set`
gains the whole coordinated phrase *"Antares or its Subcontractor"* (§3.5.1's v0.33 rider),
`C02-03`'s `object_class_accept_set` gains `invoice_costs`, and `C11-01`'s gains
`principal_interest`. **Nothing is removed from any set** — §3.4's exception is widening-only —
and **no item is restamped**: all three hold `guideline_version: v0.28`, on a measurement rather
than a preference (see §3.6's v0.62 execution table, where the widen-and-restamp alternative is
priced and refused). **In-force criterion 2 moves `3/10 = 30.0%` → `5/10 = 50.0%`**, confirmed by
a real `run_scoring.run()` before and after.

**The justification test was satisfied BEFORE execution and is restated here because it is the
one thing that could have been skipped.** None of the three arguments points at a prediction:
`invoice_costs` is a compound of two labels the set already held, `principal_interest` is the
possessor-anchored reading of *"the Principal's interest in Franchisee"*, and the coordinated
phrase is what the span literally offers. §3.6.2's retroactive no-restatement screen was re-run
over the widened set and returns a **byte-identical** result — the three new members introduce
zero restatement hits — and head-only member coverage holds at **13/36**, so neither of §3.6's
own invariants was loosened in passing.
...
**EXECUTED v0.62 — AND THE SIZING ABOVE IS A DATED RECORD, RECONFIRMED RATHER THAN ASSUMED.**
The `/9` denominator above predates v0.59's kind-scoping, which moved it to `/10`. F1 was
therefore re-measured against the committed set at execution time rather than carried forward on
the v0.58 arithmetic, and **the `+2` reproduces exactly**:

| | in-force criterion 2 | all-items | movers |
| :--- | :--- | :--- | :--- |
| baseline at execution | `3 / 10 = 30.0%` | `5 / 15 = 33.3%` | — |
| **F1 applied (committed)** | **`5 / 10 = 50.0%`** | **`7 / 15 = 46.7%`** | `C02-01`, `C02-03` |
| rejected alternative (widen **and** restamp) | `3 / 7 = 42.9%` | `5 / 12 = 41.7%` | *none — both movers drop out* |

**The rejected row is the load-bearing one and it was measured, not argued.** Restamping the
three stales their cassettes, so both movers leave the denominator entirely: the numerator does
not move at all and the figure "improves" `30.0% → 42.9%` **purely by shrinking**. That is
exactly the shape F19 refused — *"a published figure improving for no improvement"* — so all
three stamps are **held at `v0.28`**. Scoreability is unchanged across the widening: 36 items,
**15 scored / 21 cassette-unscoreable** before and after, so nothing here is bought by dropping
an item.

**`C11-01` is worth zero exactly as predicted, and that prediction is now confirmed rather than
restated**: its modal outcome stays `PARTIAL`, with the failing-clause list going
`['object_class', 'conditions', 'obligee']` → `['conditions', 'obligee']`.

**A SECOND-INSTRUMENT EFFECT THE v0.58 SIZING DID NOT MEASURE, disclosed here rather than left
to be found.** §3.6.1's v0.45 requirement is that a retroactive edit state its effect on **both**
predicates, and the sizing above priced only §5. Measured at execution: **`K` is UNMOVED at
`14/32`** (verdict `REDESIGN` unchanged) and **in-force `A` is UNMOVED at `7/27`** — but the
**published legacy per-clause profile** would have drifted `5_object` **6 → 5 of 23**, via
`C11-01`, because `A` compares slots by mutual membership and reads gold's set directly where
§5 clause 5 never reads gold's slot. That drift is a change to a **dated, published** run, not to
a current figure, so it is held by the same `PRE_RESTAMP` mechanism §10.1 F11 and F13 already
use, extended to a widening rather than a restamp. **The widening is not weakened and the
published run still reproduces exactly, item by item and clause by clause.**
...
**A reviewable-standard phrase is NOT a `conditions` entry (v0.54 — REVIEWER-RULED, first
instance: `C04-06`).** A phrase that states the **standard by which a party's exercise of a
right or duty is judged** — *"acting reasonably"*, and by the same logic *"in good faith"*,
*"to its reasonable satisfaction"* — governs **how** the act may be performed, not **whether**
the obligation applies. `conditions` holds the circumstances the obligation *depends on*, so a
standard belongs in neither the list nor its count. `C04-06`'s *"Miltenyi, **acting reasonably**,
reserves the right to defer…"* is annotated `conditions: []`, and the phrase does its real work
in **§2.5.1**, where it is that section's own named discriminator against unreviewable discretion
(*"sole business judgment"*) and is what makes the clause eligible at all. **The same phrase
deciding eligibility and being excluded from `conditions` is the point, not a tension:** §2.5.1
asks whether the clause can ever be checked, §3.8 asks what must be true before it bites. Keep
`conditions` for the latter. Recorded so the next instance is decided by citation rather than
re-derived — no locked item before `C04-06` had reached the question.
...
| `EVERY` + `DURING` composed | **CORRECTED v0.61 (F19) — this row stated the GRAMMAR path and was read as the pipeline's. On the EXTRACTION path it is `UNMAPPABLE_TEMPORAL`, whole candidate rejected**: `_EVERY_RE` is `$`-anchored after the unit, so `every 30 days during …` matches no form at all. `EVERY`-only-with-a-warning is real but reachable only from a hand-built DSL string, which extraction never produces (§8.11) |
| Lead-time deadline (`at least N <unit> before <trigger>`) | Unrepresentable — **no v1 form pairs a `duration` with a `direction`**, so the lead time has nowhere to go and composition does not rescue it (§8.11) |
...
**CONFIRMED AND THE FLAG DISCHARGED at v0.64 — see §8.8.5 (§10.1 F22).** The single-verb
accept-set stands; the empty-set alternative is re-priced with v0.59's kind-scoping (which has
since removed every item in this class from §9.1's in-force denominator, so its v0.24 cost lands
only on the all-items figure) and still rejected; the narrow "widen among near-synonyms of the
nearest verb" fix is rejected on a measurement — it admits **nothing** on 4 of 4 items that
recorded a runner-up. The complaint stated in the paragraph above is **not** dismissed: it is
answered by a reporting rule (§8.8.5's G9), not by a data change. **The paragraph above is left
as written and is a dated record of the open question, not live status.**
...
#### 8.8.5 The single-verb accept-set is CONFIRMED, and §14.3 never reached this class (v0.64 — REVIEWER-RULED, §10.1 F22)

**What was open, in this section's own words.** §8.8's SUB-CHOICE paragraph is marked *"v0.24
default, pending confirmation"* and states its own complaint plainly: *"The single-verb
accept-set means an extractor scores clause 2 correct only by picking the SAME near-miss the
annotator picked; §14.4's objection to defaults partly applies."* It named one alternative — an
**empty** `action_accept_set` — and deferred: *"Revisit once batch 3 supplies a second rate."*
Batch 3 supplied two (`C10-02`, `C14-04`); `E08-04` is the third, and its segment file logs the
collision as a forcing function. **Ruled here: the default is CONFIRMED and the flag is
discharged — but for a reason v0.24 did not have, and the collision it was filed under does not
exist.**

##### The class is EIGHT items, not three — measured before anything was ruled about it

| item | real verb | slot | accept-set | `annotator_confidence` |
| :--- | :--- | :--- | :--- | :--- |
| `C10-01` | *defend* (+ *hold harmless*) | `INDEMNIFY` | `['INDEMNIFY']` | `AMBIGUOUS` |
| `C10-02` | *add* | `ESTABLISH` | `['ESTABLISH']` | `AMBIGUOUS` |
| `C14-04` | *perform … to complete* | `PROCESS` | `['PROCESS']` | `AMBIGUOUS` |
| `E01-01` | *amend, supplement, modify* | `WAIVE` | `['WAIVE']` | `AMBIGUOUS` |
| `C04-06` | *defer* | `WITHHOLD` | `['WITHHOLD']` | `CONFIDENT` |
| `C02-07` | *cease and desist* | `PROCESS` | `['PROCESS']` | `CONFIDENT` |
| `E03-03` | *guarantees* | `ENSURE` | `['ENSURE']` | `CONFIDENT` |
| `E08-04` | *dispute* | `PROCESS` | `['PROCESS']` | `CONFIDENT` |

**8 of 48 locked items, 16.7%.** Every one already carries a singleton, so the rule is applied
uniformly across eight items and the three logged instances are only where the collision was
**noticed**. The 16.7% sits inside §8.8's sampled 23%, 95% Wilson CI [11%, 42%]; it is recorded
as a corroborating observation and **the sampled rate is NOT restated**, on §8.8.2's own ground
that a census of locked items does not move a sample-based estimate.

##### THE COLLISION IS A ROUTING ERROR AT §14.1, NOT A CONFLICT BETWEEN TWO LIVE RULES

§14.3 routes an ambiguity *"in an accept-set field"* to a widening. §8.8's genuine-gap branch
forbids widening. They look like they both fire on the same item. **They do not, because
§14.3's precondition is not met.** §14.1 defines `AMBIGUOUS` as *"**The sentence** admits ≥2
defensible readings that yield different gold values"* and glosses the distinction in terms:
*"`AMBIGUOUS` is a property of the text. `UNCERTAIN` is a property of the annotator."*

In a genuine-gap item the **text** is not in doubt. `C10-02`'s sentence says *add*, `C14-04`'s
says *complete*, `E08-04`'s says *dispute* — no annotator disputes what the duty is. What is
in doubt is the **encoding**: which member of a closed list to settle for once it is already
established that none of them denotes the performance. That is §14.1's `UNCERTAIN`, whose route
is §14.4. **And it is the route all three in fact took** — every one carries
`adjudicated_by: reviewer`, and `E03-03`'s notes state the reasoning explicitly for a different
field (*"the ambiguity is not in an accept-set field, so §14.3 routed it to §14.4 rather than to
a widening"*).

##### A SECOND GROUND, which does not depend on that reading — and `C10-02` is why it is stated

**Disclosed rather than glossed: the routing argument is strong for `C14-04` and `E08-04` and
GENUINELY ARGUABLE for `C10-02`.** Its notes weigh `ESTABLISH` (the literal act, *"set up an
entry"*) against `GRANT` (*"adding a party to an insurance certificate does confer
additional-insured status"*), and a reader can fairly call that two readings of the sentence
rather than two encodings of one reading. The ruling does not rest on defeating that.

**§14.3's remedy presupposes that each competing reading is REPRESENTABLE.** Widening records a
fact about the sentence: it admits both, and both are expressible, so the accept-set is the
honest encoding of a real ambiguity — the eval pilot's `PROVIDE`/`REPORT` case for *"submit a
compliance report"*, where **both verbs are correct**. In a genuine gap **none** of the
candidates is correct; the annotator has already ruled that, and the tag is that ruling written
into the data. Widening there does not record what the sentence admits. It records the
annotator's own indecision among known-wrong answers — which is **§14.2's prohibition** (*"Do
not mark an item `underspecified` because you are unsure of the answer… it is a scored field in
the conjunctive predicate"*) applied to clause 2 instead of clause 8.

**The principle, promoted from §14.2 rather than authored here**, in the same shape §8.6.1 took:

> **`action_accept_set` records what the SENTENCE admits, never what the TAXONOMY forces the
> annotator to settle for.**

##### THE NARROW FIX IS REJECTED — ON A MEASUREMENT, NOT A PREFERENCE

The candidate put to this ruling was *"permit widening only among near-synonyms of the nearest
verb, rather than across the full accept-set."* It is worth stating why it is attractive: it
preserves §8.8's *"why not generous accept-sets"* purpose while removing the part §14.3 objects
to. **Both available readings of it collapse, and the first collapses on its own motivating
items.**

**Reading 1 — near-synonyms of the NEAREST verb (the verb in `action`). Measured across every
item in the class that recorded a runner-up, it admits NOTHING:**

| item | nearest | runner-up the annotator actually weighed | near-synonym of the nearest? |
| :--- | :--- | :--- | :--- |
| `C10-02` | `ESTABLISH` | `GRANT` — the notes' *"strongest competitor"*, taken on the legal **effect** | no |
| `C14-04` | `PROCESS` | `DELIVER` | no |
| `E08-04` | `PROCESS` | `COMPLY`, `NEGOTIATE` | no |
| `E03-03` | `ENSURE` | `GUARANTEE`, `WARRANT`, `ASSURE` | **yes — and all three are OFF-TAXONOMY** |

**4 of 4: zero admitted.** The structural reason is visible in the table and is not an artifact
of these four. A runner-up in a genuine-gap item is a **competitor** to the nearest verb — a
different bad fit reached from a different angle — not a synonym of it; and where genuine
synonyms of the nearest verb do exist, they are outside `ACTIONS`, which is what made the item a
gap in the first place. **The fix is not a smaller version of widening. It is the status quo
with extra machinery**, and on `C10-02` it excludes the very competitor that motivated it.

**Reading 2 — near-synonyms of the REAL verb.** That is §8.8.2's mappability test (*"does a
taxonomy member denote the SAME PERFORMANCE?"*) verbatim, and it returns **NO for the whole
class by construction** — a YES is exactly what makes an item defensibly mappable and untagged.

**And it has no mechanical test.** §8.8.2's *"same performance?"* is the only verb-closeness
relation this guideline owns. The narrow fix needs a relation weaker than that and stronger than
*"any taxonomy verb"*, and none exists. Authoring one would trade a checkable boundary for an
unfalsifiable one — **the identical objection §8.3.2 already sustained** when it refused to
extend branch 2 to a *"feels like a legal doublet"* judgment, and the same preference §2.5.1,
§3.8.2 and §5's number rule each record.

##### THE EMPTY-SET ALTERNATIVE IS PRICED WITH THE FACT v0.24 LACKED, AND STILL REJECTED

Stated against this ruling's own conclusion, because the alternative has become materially
cheaper since it was deferred and a confirmation that hid that would be worthless.

**What changed: v0.59's kind-scoping (§8.10, F9) removed every item in this class from the
criterion that matters.** `action_not_in_taxonomy` is `REPRESENTATIONAL`, so §9.1's in-force
denominator already excludes all eight. §8.8's decisive objection in v0.24 — *"converts a
measurement question into a guaranteed failure for roughly one item in four"* — now lands
**only** on the all-items figure, which §9.1 labels *"reported alongside, never as the
criterion."* The v0.24 pricing is a dated record.

**Rejected anyway, and the ground is narrower than the v0.24 one.** Both figures are recoverable
from the committed data under either option: the `action` slot holds the nearest verb whether
the set is `[that verb]` or `[]`, so an analyst wanting the empty-set number computes it from
the tag, and one wanting the singleton number computes it from the slot. **Nothing is destroyed
either way, so the choice is only about which figure is the DEFAULT reading** — and the
singleton keeps clause 2 answering a real, if low-value, question (*did the model settle on the
same near-miss?*) where the empty set makes it answer nothing at all. A tie goes to the rule
already in force and already applied uniformly to eight items.

##### WHAT IS ADDED, because a confirmation must not leave the section's own complaint unanswered

The SUB-CHOICE paragraph's objection is **legitimate and survives this ruling**: on a
genuine-gap item clause 2 is not measuring what clause 2 exists to measure. Confirming the rule
without saying so anywhere the number appears would bank the complaint and publish the figure.

**Rule: wherever criterion-2 figures are reported, clause 2 on an `action_not_in_taxonomy` item
is disclosed as a FORCED-CHOICE AGREEMENT CHECK, not an action-identification measurement**, and
the affected items are named on the spot. This is the honest half of the empty-set alternative
without its data change, and it is the same on-the-spot discipline §9.1's G6 and §6.1 already
require, for the same reason: a reader who does not reach the methods section still gets the
caveat. Implemented as `report.py`'s **G9**.

##### COST — MEASURED ON FOUR INDEPENDENT GROUNDS, EACH VERIFIED BY EXECUTION

**No published figure moves, and a widening on any of the eight would not have moved one
either.** The second half matters more than the first: it means this ruling could not have been
reached for to protect a number.

| instrument | why it cannot move | how it was checked |
| :--- | :--- | :--- |
| **in-force criterion 2** | `action_not_in_taxonomy` is `REPRESENTATIONAL`; §9.1's kind table excludes it | `gap_kinds.GAP_KIND` read by execution |
| **all-items criterion 2** | **ALL EIGHT tagged items are cassette-unscoreable today**, for three different reasons, so none of them reaches either denominator | a real `run_scoring.run()` names each one: no cassette at all (`C10-01`, `C10-02`, `C14-04`, `C02-07`, `E03-03`), no committed §21 R1-R3 registry (`C10-01`, `C10-02`, `E08-04`), and a §22.3 guideline-version mismatch on an existing cassette (`E01-01` at `v0.48` against `v0.28`; `C04-06` at `v0.53` against `v0.48`). **Two of the eight ARE cassette-backed — `E01-01` and `C04-06` — and a first draft of this row said otherwise**; corrected by reading the run's own G8 list rather than the cassette directory |
| **`A` (`7/27`) and `G`** | `C10-02` and `C14-04` are `NON_CONFORMING` under §5.1's A1 gate — cold's paired slots are `INSURE` and `COMPLETE`, both off-taxonomy — so they sit outside **both** sides of the fraction and were already removed from `G`. `E08-04` is outside the published population entirely | `annotator_agreement.compute()` returns `Agreement.NON_CONFORMING` for both |
| **legacy published `K` (`14/32`)** | both fail `2_action` today and **no LEGAL widening can flip it**: legacy clause 2 is `cold_slot ∈ gold_set ∪ {gold_slot}`, cold's slots are off-taxonomy, and §3.4 admits only **taxonomy** verbs into a gold accept-set. A structural guarantee, not a measurement of one option | `compare_clauses(..., symmetric=False)` read directly, plus the two cold slots checked against the real 34-verb `ACTIONS` |

Baseline and post-ruling both confirmed by a real `run_scoring.run()`: **in-force criterion 2
`5/10 = 50.0%`, all-items `7/15 = 46.7%`, 48 items, 15 scored, 33 cassette-unscoreable**, `K`
`14/32`, `A` `7/27`.

##### `E08-04`'s PROVISIONAL DISPOSITION — DISCHARGED, NOT CHANGED

Its interim answer was `PROCESS` + singleton + `action_not_in_taxonomy`, ruled *"so the batch is
not blocked"* pending this section. **This ruling CONFIRMS the rule it was decided under, so
not one field changes.** What ends is the provisional status.

**NOT RESTAMPED.** It holds `guideline_version: v0.62`, on `C11-02`/`C04-06`'s v0.54 precedent:
a rule that **confirms** an item's treatment rather than changing it does not earn a restamp,
and restamping a confirmed item is the gratuitous conforming event §22.1 and `C11-01`'s own
record-only correction both refuse. Its segment has no cassette, so a restamp would be free —
**and "free" is not a reason**, which is the same discipline §22.1 applied in the opposite
direction. The item's `annotator_notes` and `E08-016`'s observation are updated record-only.

##### WHAT THIS SECTION DELIBERATELY DOES NOT DO

- **It does not correct `annotator_confidence` on the four `AMBIGUOUS` items.** Under the routing
  finding above their correct §14.1 state is arguably `UNCERTAIN`, but `C10-02`'s case is
  genuinely arguable (see the second ground), the field is **read by no code under `apps/brain`**
  — verified, `grep` exits 1 over the whole tree — and flipping provenance labels retroactively
  on an arguable reading buys nothing. The observation is recorded; the data is not touched.
- **It does not widen or narrow `ACTIONS`.** §8.8's *"why not widen"* paragraph is untouched.
- **It does not decide any individual mapping.** §8.8.2's *"same performance?"* test governs
  which items enter the class, and no item enters or leaves it here.
- **It does not resolve F21.** `action_not_in_taxonomy`'s own direction (`INCOMPLETENESS`) is
  uniform across its members and was never the mixed case F21 was opened for.
...
| :--- | :--- | :--- | :--- | :--- |
| **`REPRESENTATIONAL`** | IR v1 has **no form** for what the document says; the loss is baked into the gold item itself | **required** | **no** — knowingly unfaithful | `exception_unsupported`, `unless_unsupported`, `compound_action`, `mutual_obligation`, `action_not_in_taxonomy`, `lead_time_unrepresentable` (§8.11, v0.61), `temporal_composition` (§8.12, v0.63 — **direction DEFERRED**, F21) |
...
| **`WITHHELD_VALUE`** | The operative **value** was withheld in the filing — the **construction** survives | **`INCOMPLETENESS`** (v0.60, F18 — was *contested*) | **no** — see §8.10.1 | `redacted_value` (§8.1) |
...
**What a new tag must now do.** State its **kind** first. If and only if that kind is one where
**gold departs from the document** (§8.10.1 — `REPRESENTATIONAL` or `WITHHELD_VALUE`; through
v0.59 this said `REPRESENTATIONAL` alone), state its **direction** as well. **v0.63 (F21) adds a third possibility on the second axis**: a direction-bearing tag whose
direction is **ruled open** rather than unruled states `DIRECTION_DEFERRED` and says where the
question is filed — see §8.12. That is not a licence to skip the axis; it requires an argument
that the direction *varies*, not merely that it is hard. An unruled tag is `UNCLASSIFIED`, is
...
### 8.10.1 `WITHHELD_VALUE`'s direction — **RULED v0.60 (§10.1 F18); `INCOMPLETENESS`, and the kind stays EXCLUDED**

**The figure does not move. `3/10 = 30.0%` stands**, all-items `5/15`, `K` `14/32` — confirmed by
a real `run_scoring.run()` before and after, not hand-computed. What changes is that
`WITHHELD_VALUE` is now excluded **by ruling** rather than **held** pending one, and
`DENOMINATOR_EXCLUDING_KINDS` is unchanged in content.

**F18's premise is falsified by measurement, and that is what decides it.** The question was
filed as *"does gold's `null` faithfully record that the value is genuinely absent?"* The
redaction removes the **value**; it leaves the **construction** standing, and the pipeline reads
the construction. Measured on `E03-005`, all **3 of 3** runs, the model quotes the redacted phrase
into `temporal_raw` verbatim:

> `"At least ** before the ** of each Calendar Quarter during the Term of this Supply Agreement"`

`_classify_temporal` returns `None` on it → `UNMAPPABLE_TEMPORAL` → the repair loop fires. Run 1
quarantines at IoU 1.000 (`MISSED`); runs 2–3 the repair narrows it to
`"before the ** of each Calendar Quarter"`, **which classifies** — verified by execution against
the production classifier, not read off the repair text:

> `'before the ** of each Calendar Quarter'` → `BEFORE "the ** of each Calendar Quarter"`

So the prediction carries `RELATIVE_TO_TRIGGER(before, "the ** of each Calendar Quarter")` against
gold's `null`. That is `E03-01`'s scored result exactly: `PARTIAL`, `failed=['temporal',
'underspecified']`, with clause 8 purely downstream — the unresolvable trigger forces
`underspecified: true` against gold's `false`. **One root cause, both clauses, and the root cause
is the redaction rule.**

**Ground 2 bites, and §8.1 concedes its own premise.** §8.1 already names the hazard — *"that
conflates 'the contract states no deadline' with 'a deadline exists and was withheld from the
filing'"* — and answers it with `missing_fields` + `redacted_phrase`, *"the channel that keeps the
withheld-versus-absent distinction recoverable, which is this rule's actual purpose."* **That
channel is invisible at the point of certification**: §5 excludes `missing_fields` from the
predicate and `redacted_phrase` is non-scored. So what a `FULLY_CORRECT` verdict would certify is
the bare `null`, and the bare `null` discards a constraint the document plainly states.
**Recoverability from the ITEM is not recoverability from the PREDICATE, and ground 2 reads the
predicate.** Direction is therefore `INCOMPLETENESS` — the IR claims **less** than the document,
and a monitor built on `E03-01` never checks the forecast lead time at all.

**A second, independent ground, recorded separately because the two fail differently.** The item
is not winnable in practice: every prediction available from this input either quotes the redacted
timing phrase (→ non-null `temporal`, or quarantine) or drops it (→ `null`, clause 6 passes), and
across 3 runs the model has never dropped it, because the phrase is plainly present and plainly
temporal. **Stated precisely rather than overclaimed: this is measured model behaviour, not
`REACHABILITY`'s grammatical guarantee — it is the weaker of the two grounds, and ground 2 alone
is sufficient.**

**AGAINST INTEREST, and it is the sharpest thing this ruling found: on this item the model's IR is
MORE faithful to the document than gold's is.** It recovered the real structure (`before
<trigger>`) and lost only the magnitude, which is genuinely unknowable from the filing. Gold
discards the constraint entirely. **That is an argument for revisiting §8.1's `null` rule — it is
not an argument for admitting the item to the headline denominator**, which is what F18 asked, and
the two must not be run together.

**Why the kind is NOT collapsed into `REPRESENTATIONAL`.** §8.10's ground for separating them
stands untouched: **no IR change helps**. IR v1 *has* the form — the classifier produced it. The
gap is in the input, not in the representation. So the taxonomy keeps five kinds while two of them
carry a direction, which is coherent under a sharper principle than v0.59's:

> **Direction is assignable wherever GOLD DEPARTS FROM THE DOCUMENT** — `REPRESENTATIONAL` because
> the IR has no form for the clause, `WITHHELD_VALUE` because §8.1 rules the field `null` though
> the IR has the form. The other three carry none because gold does not depart at all.

v0.59's *"direction only for `REPRESENTATIONAL`"* was a **proxy** for that test, correct on the
tags then live and wrong in general. This states the test itself
(`gap_kinds.DIRECTION_BEARING_KINDS`).

**A consequence found by execution rather than reasoned about, and corrected before it shipped.**
Once `redacted_value` carries a direction, G6's `WITHHELD_VALUE` **bucket becomes unreachable**:
for any direction-bearing kind `direction_of()` returns a direction or `UNCLASSIFIED` and never
`None`, so the `else kind_of(tag)` branch cannot fire for it, and a future withheld-value tag with
no ruled direction routes to `UNCLASSIFIED` — the correct report, and the same treatment a new
`REPRESENTATIONAL` tag already gets. The bucket is **removed** rather than left in place, because
a bucket kept after it can no longer be reached reads as *"no such items"* when it in fact means
*"no such path"*. A test pins the unreachability. **The first draft of this ruling kept the bucket
with a justification that was simply false**, and it was caught by checking the claim rather than
by re-reading the code.

### 8.11 Lead-time deadlines are a SINGLE-FORM gap, not a composition gap (v0.61, F19)

`E03-01`'s own `annotator_notes` recorded that *"the temporal would fail even unredacted: the
underlying obligation is recurring-with-lead-time, plausibly `EVERY` composed with
`RELATIVE_TO_TRIGGER`, which IR v1 cannot represent (§7 composition gap)"*, flagged as **plausible**
and never verified. Checked against the real IR: **the conclusion holds and the mechanism does
not.**

**The conclusion, confirmed through the production `compile_candidate()`** (not read off the
regexes): `"At least ** before the ** of each Calendar Quarter during the Term…"` as emitted on 3
of 3 runs, and a plausible unredacted `"At least thirty (30) days before the first day of each
Calendar Quarter during the Term…"`, **both return `UNMAPPABLE_TEMPORAL`**. So the item genuinely
owes a second gap, and `known_gaps` was under-reporting.

**The mechanism, on three grounds.** *(a)* Neither claimed operand is available for this text:
`'each Calendar Quarter'` and `'every Calendar Quarter'` both classify to `None`, because
`Duration` is `{amount, unit ∈ h/d/bd/w/mo/y}` with **no `UNRESOLVED` alias variant** (unlike
`DateRef`/`TriggerRef`), so a recurrence named by a defined term has no slot; only the paraphrase
`'every 3 months'` classifies, and gold never performed that constant-fold. *(b)* The load-bearing
failure is **single-form expressiveness**. A mechanical slot check over all five forms:
...
Forms carrying `{duration, direction, trigger}`: **NONE**. Forms carrying `duration` **and**
`direction`: **NONE**. `at least ** before the **` is one atomic timing element needing all three.
Confirmed: `'at least 30 days before …'` → `None`, `'30 days before …'` → `None`, and only the
duration-**stripped** `'before the first day of each Calendar Quarter'` classifies. *(c)* The note
omits `during the Term` entirely — §7's *actual* named operand.

**THE DECISIVE TEST, and it is why §7 is the wrong citation: granting free composition of all five
v1 forms does NOT rescue this clause.** Composition joins whole forms, and no form holds the
`{duration, direction}` pair a lead time needs. The nearest composable approximation,
...
**Hence a tag of its own, `lead_time_unrepresentable`** — kind `REPRESENTATIONAL` on §8.6.1's test
(the IR has no form; this is not a surface pattern rejecting input the IR could hold), direction
**`INCOMPLETENESS`**, reached independently of the note: the IR drops the lead-time duration and
keeps at most the bare direction, so a monitor never checks the advance-notice period and misses a
real breach. `OVERSTATING` was considered and rejected on the semantics above.

**THE TWO ANNOTATORS' AGREEMENT ON `redacted_value` IS NOT CORROBORATION, and this is recorded
because it reads as corroboration at a glance.** Cold's `known_gaps` for this item is
`["redacted_value"]`, identical to gold — but cold **explicitly considered and declined** the
composed reading, and identified the composed element as **`during the Term` (`DURING`)** where
gold's note says **`EVERY` ∘ `RELATIVE_TO_TRIGGER`**. Two annotators, two *different* and mutually
inconsistent characterisations of one clause, converging on one tag. Set equality on `known_gaps`
is agreement about a **tag**, never about the **mechanism** behind it — which is the same blind
spot §7.1 already names for `K`, reached one level down.

**Cold's decision was influenced by a guideline error, which is why §8's row is corrected rather
than merely annotated.** Cold's note reasons: *"§8's gap table records … an `EVERY`+`DURING`
composition as `EVERY`-only-with-warning — so no reading of it changes `temporal` from null here."*
That row stated the **grammar** path. On the **extraction** path `every 30 days during …` matches
`_EVERY_RE` not at all (`$`-anchored after the unit) and the whole candidate is rejected as
`UNMAPPABLE_TEMPORAL` — verified end to end, alongside a confirmation that the grammar path *does*
still warn, so the guarantee is real and simply unreachable from extraction.

**THIS IS THE FOURTH TRACKED INSTANCE of a guarantee proven at the grammar/DSL layer that the real
extraction path bypasses**, after (1) `UNLESS` absorption (§8.2 / `SPEC.md` §6), (2)
`ir_compile.py` never building an `AndPredicate`/`OrPredicate` despite AST and grammar support
(§17.3), and (3) the IR-Compiler trailing-period bug. The pattern's signature is constant: the
guarantee is real where it was tested, the extraction path never reaches that layer, and every
instance was found by **testing rather than by reading**. Unlike the first three it is recorded
here as a *documentation* defect as well as a code one — the row was not merely wrong, it was
**cited by an annotator as a reason not to tag**, so a layer confusion in a reference table
propagated into the gold set itself.


### 8.12 Multi-element temporals are a SLOT gap, not a form gap — and the direction axis cannot answer for them (v0.63, F21)

**Two logged instances forced this, and they are the forcing function §22.1's discipline
asks for — a kind is not ruled on a sample of one.** `C04-051#temporal-composition`
(instance one, `exclusions.json`) and `C04-072#temporal-composition` (instance two,
`segments/C04-072.json`) each record a clause whose timing is **three** elements. Both were
logged rather than resolved inside an annotation pass, and both are corrected here on points
that did not survive execution.

**The rule. `temporal_composition`, kind `REPRESENTATIONAL`, direction DEFERRED BY RULING
(F21).** It fires when a span states **two or more temporal elements that constrain the duty
differently** and IR v1's single `temporal` field can hold at most one. It does **not** fire on
a redundant restatement of the same bound, which loses nothing.

**Why it is not §8.11, restated on the corrected evidence.** §8.11's decisive test was that
*"granting free composition of all five v1 forms does NOT rescue the clause"*, because no form
pairs a `duration` with a `direction`. Here the test comes out the other way — but **only on
instance two**, and both logged notes overclaim it. Measured through the production
`_classify_temporal()`:

| element | classifies to |
| :--- | :--- |
| `after expiry of the said 6-month period` | `AFTER "expiry of the said 6-month period"` |
| `during the Term of this Agreement` | **`None`** |
| `until Bellicum has secured an alternate source…` | **`None`** |
| `within 30 days of the Effective Date` | `WITHIN 30d OF "the Effective Date"` |
| `by 31 December of each Calendar Year` | `BY "31 December of each Calendar Year"` |

So *"each element has a v1 form on its own"* is **false as written in both notes**: one of three
for instance one, two of three for instance two. Instance one's own note measures the two `None`s
a few lines below the claim without reconciling them. The distinction from §8.11 **survives on
instance two**, where free composition genuinely would rescue the deadline pair, and instance one
is a **mixed** case: `until <event>` has no v1 form at any layer — `DuringTemporal` is
`{start: DateRef, end: DateRef}`, so there is no end-only slot and no trigger-typed terminus — so
that element is itself §8.11-shaped. **The sibling is `compound_action` (§8.3), not
`lead_time_unrepresentable`:** one slot, several stated members, and the tag records what the slot
cannot hold.

**Kind is `REPRESENTATIONAL` and the alternative is ruled out rather than waved past.** Not
`REACHABILITY`: nothing is rejected — both instances **compile**, and a perfect classifier could
not help, because `ast.Obligation` carries one `temporal` field. Not `CORPUS_DEFECT` or
`ANNOTATION_CONVENTION`: neither the text nor a gold rule is at fault. §8.6.1's test applies
directly — this is what the IR cannot **represent**, not what the classifier cannot **reach**.

#### The scope is the SLOT LIMIT, not the greedy swallow

Both notes describe a *"greedy `(.+)$` capture"* swallowing the surplus bounds into the trigger
string, and instance two rightly observes that the shared trailing capture makes this a property
of the classifier's **shape** rather than of one pattern. **That is a symptom, not the defining
condition**, and it is confined to the cases where the surplus sits **inside** the quoted phrase.
Scoping the tag by the swallow would make a `REPRESENTATIONAL` tag record a **classifier
artefact**, which is incoherent. So the tag is scoped by the slot limit, with two consequences
recorded explicitly:

- **`C02-047`'s logged near-miss IS in scope**, and `C02-07` is tagged. Its note's mechanical
  observation stands and was re-verified — the terminal bound sits outside the annotated phrase,
  nothing is swallowed, and `until a decision by AMAG has been made…` classifies to `None` — and
  its refusal to inflate the instance count stands too. **What the ruling separates is the
  question that note ran together: what counts as an INSTANCE for the forcing function is not
  what falls in the TAG'S SCOPE.** The instance count behind this amendment is still **two**.
- **`E03-01` is swept in**, from §8.11's own observation that the F19 note *"omits `during the
  Term` entirely"*. That omission is not just a defect in the note: it is a second temporal
  element, and the two tags are **distinct with neither subsuming the other** — §8.11 says the
  lead-time element has no form at all, §8.12 says only one element can reach the slot even if
  every element had one. §9's per-tag counts are already non-summable.

#### Direction is DEFERRED BY RULING, and the ground the notes gave for deferring it is the wrong one

Both notes leave direction open because *"the greedy-swallow yields a trigger string asserting a
WRONG window rather than a missing one, which is closer to a misstatement."* That measures the
**pipeline's** output. §8.10.1's test is whether **gold** departs from the document, and gold
never swallows — it records one element and drops the rest. **"Misstatement" is therefore not a
candidate direction here at all**, and no third value is minted for it.

**Direction is deferred on a different and stronger ground: it varies by instance, and within a
single item.**

| dropped element | what gold's IR then claims | shape |
| :--- | :--- | :--- |
| a **narrowing** bound — `during the Term`, `until an alternate source is secured` | the duty subsists longer than the contract imposes; a monitor flags a breach the contract exempts | **OVERSTATING** |
| an **additive** deadline — `thereafter by 31 December of each Calendar Year` | a required performance disappears; a monitor misses a real breach | **INCOMPLETENESS** |

`C04-08` drops **one of each**. A tag-level value would therefore certify a direction that does
not describe what is actually wrong with a specific item — the failure §9.1 ground 2 exists to
prevent. **An OVERSTATING assignment was drafted and rejected**, on evidence that looked adequate
and is not: it is attested in 2 of 2 instances (both drop a scope bound), it matches
`unless_unsupported`'s own ground (a dropped narrowing bound is a carve-out in the time
dimension), and in instance one gold has **no choice** — its narrowing bounds are exactly the
unrepresentable ones. **Winning 2-of-2 on instance count is not the same as describing the class**,
and minting a wrong-in-general rule to close out a session is the thing this queue exists to avoid.

**So the vocabulary gains a third state on axis 2: `DIRECTION_DEFERRED`
(`gap_kinds.DIRECTION_DEFERRED`).** It is spelled **explicitly**, never as a missing
`GAP_DIRECTION` entry, for the same reason §8.10 keeps `None` and `UNCLASSIFIED` apart one level
up: an absence is indistinguishable from *"someone forgot"*, and a deliberate deferral must not
be a place an unruled tag can hide. Three answers now, all distinct: a direction; deferred by
ruling; unruled. G6 gains a matching bucket.

**This costs nothing today, which is why deferring is available at all.** §9.1's denominator reads
**kind**, not direction, and `REPRESENTATIONAL` excludes on its own — so the tag excludes its
items without a direction having to do that work. **All three items were already outside the
in-force denominator on existing tags** (`redacted_value`, `action_not_in_taxonomy` /
`relative_trigger_preposition`, `redacted_value` + `lead_time_unrepresentable`), so the amendment
is **exclusion-neutral**, confirmed by a real `run_scoring.run()` before and after.

**Second-instrument effects, measured rather than assumed (§3.6.1's v0.45 dual-predicate rule).**
`K` holds at `14/32`, in-force `A` at `7/27`, the F17 band at `(16, 17)`, and — unlike F19, which
moved it — **`G` holds at `7/31`**. The reason is structural rather than lucky, which matters
because v0.61 found `G` had been matching its published figure by two drifts cancelling:
`E03-01`'s pair is **already** in `superset_items` (gold ⊃ cold), so a further gold-only tag
cannot change which class it falls in. The dated 2026-08-29 reproduction is untouched, held by
the same `PRE_TAG` map. **`A` is unmoved only because the tag was added to the SECOND vocabulary
copy in the same commit** — v0.61's own trap, where a tag reaching `gap_kinds.GAP_KIND` alone
left `annotator_agreement.SECTION_8_TAGS` stale and silently marked its item `NON_CONFORMING` in
a different instrument. Both are pinned by tests.

**Not restamped, on F19's precedent.** `known_gaps` is not one of §5's eight scored clauses, so
none of this can change how an item scores; `E03-01` keeps `v0.28` and the batch-4 pair keep
`v0.62`. `C02-07` and `C04-08` have no cassette at all, and restamping `E03-01` would stale
...
### 8.13 `within_parenthetical` is MINTED, kind `REACHABILITY` — and §11's tag had never existed in code (v0.65)

**§11 decision 1 has instructed, in the imperative, since v0.8: *"Every such item is tagged
`known_gaps: ["within_parenthetical"]` so the two denominators are computed mechanically rather
than reconstructed by hand afterward."* It was a PAPER TAG.** Verified by execution against the
committed harness rather than by reading: it is absent from `gap_kinds.GAP_KIND`, absent from
`annotator_agreement.SECTION_8_TAGS`, and carried by **zero** of the 48 locked items. Under §8.10's
forcing function the first item to use it would have **failed a test**, because no `kind` had ever
been ruled for it. Minted here because batch 5's `E02-02` is that first item.

**Kind is `REACHABILITY`, on §8.6.1's own test, verified by execution and not read off the regex.**
`_WITHIN_RE`'s numeral group is `(\d+(?:\.\d+)?)`, which cannot read the parenthetical form. But
`within 3 days of receipt of the billing data` → `WITHIN 3d OF "receipt of the billing data"`, so
**the IR HAS the form and only the surface pattern rejects the input.** That is a reachability
defect in one regex, not a representational gap — §8.6.1's *"`temporal: null` marks what the IR
cannot REPRESENT, never what the classifier cannot REACH"*, applied to its sibling sub-pattern.

Consequences follow from the kind and are not separately chosen: **no direction** (gold is
faithful — it annotates the `WITHIN` form exactly as the document writes it, so nothing departs
from the contract and there is nothing to assign; `None`, a **ruling**, never `UNCLASSIFIED`);
**excluded from §9.1's in-force denominator** on the **third** ground stated there — a
structurally guaranteed compile failure, so admitting it would seat an unwinnable item in the
headline figure. **Identical treatment to `within_preposition` in every respect**, which is the
point: they are two gaps in one regex, and the ruling is that they are two *tags*, not one.

**Expected v1 outcome: `UNMAPPABLE_TEMPORAL`, a LOUD failure visible in criterion 1b** — §8.9's
side of §15.3's line, and word for word what §8.6.1 and §11 decision 1 each state for their own gap.

#### §11's tag and §8.6's "two gaps, one tag" practice are RECONCILED here, not silently merged

**They contradict each other, and the contradiction is the same shape as F13's** — two live rule
statements giving different instructions about the same regex, each internally consistent, neither
aware of the other:

| | says | on |
| :--- | :--- | :--- |
| **§11 decision 1** (v0.8) | *"Every such item is tagged `known_gaps: ["within_parenthetical"]`"* | the parenthetical numeral |
| **§8.6's v0.55 note** | `C11-03` *"trips **both** … and only `within_preposition` is tagged — exactly as §8.6 requires"* | a phrase tripping the numeral gap **and** the preposition gap |

CLAUDE.md's own `C11-03` entry states the premise §8.6 was operating on in terms: *"the numeral gap
has no tag."* **That was true as a fact about the code and false as a reading of §11**, and the two
were never put side by side.

**Ruled: both survive, because they answer different questions.** §11 says the class is **tagged**.
§8.6's note says that where a phrase trips **two** gaps, **one** tag is filed. Neither is amended:

> **A `WITHIN` phrase files the tag for the gap that is the ITEM'S OWN least-repairable obstacle,
> and exactly one tag.** Where the preposition is rejected, `within_preposition` governs and
> absorbs any co-present numeral or unit defect — `C11-03`'s disposition, **unchanged and not
> restamped**. Where the preposition is accepted and the numeral is not, `within_parenthetical`
> governs — `E02-02`'s disposition, and the first time that cell has ever been occupied.

**§10 re-check, logged rather than asserted, and it was gated on a known answer first** (the sweep
must find `C11-03` and `C17-02`, §8.6.1's two named `WITHIN` items, and it does). Over all 48
locked items, four spans carry a `WITHIN` deadline:

| item | phrase | rejecting sub-pattern(s) | disposition |
| :--- | :--- | :--- | :--- |
| `C11-03` | *within twelve (12) months **from*** | numeral + preposition | `within_preposition` alone — **unchanged** |
| `C17-02` | *within 24 months **following*** | preposition only | `within_preposition` — unchanged |
| `E08-03` | *within 30 days **of*** | **none — compiles** | untagged, unchanged |
| `C11-02` | *within the twelve (12) month period* | numeral + preposition | **no tag owed** — see below |

**`C11-02` is a screen false positive and was ADJUDICATED rather than reported.** Its phrase sits
inside the `if`-clause (*"If the conveyance … has not taken place within the twelve (12) month
period"*), and its own `temporal` is `null`: the duty it annotates carries no timing of its own.
A `known_gaps` entry records what the item's **IR** cannot hold, so no tag is owed. The span-scoped
screen over-fires here by design — §19.7's high-recall, low-precision posture — and this is the
adjudicated answer, pinned by a test. **`C11-02` is one of the ten items in §9.1's in-force
denominator, so had this gone the other way it would have moved this phase's headline figure.**

**EXPOSURE IS ONE ITEM AND IT IS THE NEW ONE. No item is restamped, no cassette is staled, and no
published figure moves.**

#### The corpus measurement the ruling rests on

Every `WITHIN` deadline in a modal sentence across all 28 documents — **179** — decomposed by which
sub-pattern rejects it. The screen is preserved at
`evals/goldens/holdout/within_parenthetical/screen.py` and is gated **twice** before any total is
read off it (Standing Principle 7): six named known answers, **and** exact reconciliation with
`corpus.py`'s own published bare/parenthetical split (`20 / 159`).

| rejecting sub-pattern(s) | n | % | documents |
| :--- | --: | --: | --: |
| numeral + preposition | 103 | 57.5% | 21 |
| numeral only | 48 | 26.8% | 20 |
| preposition only | 12 | 6.7% | 8 |
| **accepted — compiles** | **8** | **4.5%** | **4** |
| numeral + unit + preposition | 6 | 3.4% | 4 |
| numeral + unit | 2 | 1.1% | 2 |

**The numeral gap reaches 159 of 179 (88.8%) across 22 documents, and only 8 of 179 `WITHIN`
deadlines in this corpus compile at all.** That is the scale §11's still-open widening decision is
being made against, and it is why the tag was worth minting rather than folding into a neighbour.

**THE FIRST DRAFT OF THAT SCREEN FAILED ITS OWN SECOND GATE, and the failure is kept in the file
because it is the instructive one.** It captured *"three (3) calendar"* as a single numeral blob,
so the unit class came back **zero** — a clean zero that **reproduced `corpus.py`'s conflation
instead of measuring it**. Standing Principle 7's own clause: *"a zero is exactly as much a
detector output as a positive count, and reads more reassuringly."*

#### A measurement defect in `corpus.py`, REPORTED and NOT fixed

`classify_within()` reconstructs a probe `within {blob} {unit} of X` and buckets **any** rejection
as parenthetical, so the 8 unit-modifier instances are counted as numeral ones. **`corpus.py`'s
published `within_parenthetical` statistic is therefore inflated by 8 of 159 (5.0%)**, and §11's
scope decision is made against that number. This is precisely the objection **§8.6 raised against
itself** when it refused to fold into §8's parenthetical row: *"filing them together would inflate
the parenthetical gap's measured size with cases that are trivially fixable — and §11's scope
decision is being made against that number."* Not fixed here: it is a change to a metric definition
that feeds a published manifest figure, which is its own decision.

#### The calendar-days unit gap is NOT minted — §10.1 F23

`_UNIT_ALTERNATION` has no entry for `calendar days`. It is a **third** `_WITHIN_RE` reachability
defect, mechanistically distinct from both existing ones — a different sub-pattern with a different
fix (a one-word alternation widening against genuine numeral parsing) — and it is documented
nowhere: not in §8's table, not in §11, not in §8.6.

**It is not minted, on a measurement rather than a preference: 8 instances across 6 documents
(`calendar days` ×7, `continuous months` ×1), and ZERO of them occur without the numeral gap.**
There is no `unit` or `unit + preposition` cell at all. A separate tag could never fire alone on
any evidence that exists, and repairing the unit alternation alone would fix **zero** phrases in
this corpus. §22.1's discipline applies directly — a kind is not ruled on a sample of one, and
`E02-02` would be that sample.

**Forcing function, stated so it is not re-derived: a SECOND independent instance, or — far more
informative — an instance where the unit gap occurs WITHOUT the numeral gap**, which has never
happened in 179 deadlines and is the only evidence that would give a separate tag discriminating
power. Logged as observation `E02-006#calendar-days-unit-gap` and queued as **§10.1 F23**. A test
pins that the tag stays out of **both** vocabularies, so minting one later is a visible decision.

#### Cost — measured by a real `run_scoring.run()` before and after, never hand-computed

**No published figure moves. In-force criterion 2 `5/10 = 50.0%`, all-items `7/15 = 46.7%`,
`K` `14/32`, `A` `7/27`** — and `E02-02` could not have moved one in either direction, because
`E02-006` has no cassette and the item is disclosed cassette-unscoreable by G8 like every other
batch-4 and batch-5 item. **Both vocabulary copies were written in the same commit**, v0.61's own
trap, and a test pins each. Three planted defects were each **verified RED before green** and each
one *executes* rather than failing to compile — §8.8.5's lesson that *"a defect that fails to
compile is not a red test, it is an absent one"*: giving the tag a direction, removing it from
`SECTION_8_TAGS` alone, and minting the calendar-days tag.
...
| `WITHHELD_VALUE` | **excluded — RULED v0.60 (F18), no longer held** | **ground 2**, on the direction ruled at §8.10.1 (`INCOMPLETENESS`): §8.1 sets the field `null` for a clause the document states **is** constrained, and §5 cannot see the `missing_fields`/`redacted_phrase` channel §8.1 makes it recoverable through — so the bare `null` is what a `FULLY_CORRECT` verdict would certify. A **second, weaker** ground is recorded separately: measured on `E03-005`, the item is not winnable in practice (3 of 3 runs quote the redacted timing phrase; the repaired narrowing classifies). **Membership is unchanged, so the figure does NOT move** — `3/10 = 30.0%` stands, and the rejected alternative would have read `3/11 = 27.3%` |
...
| :-- | :--- | :--- | :--- | :--- |
| **F1** | `C02-03`, `C11-01`, `C02-01` | **EXECUTED v0.62 — ALL THREE WIDENINGS LANDED AS ONE ATOMIC EVENT, AND THIS IS THE FIRST QUEUE ENTRY SINCE F9 TO MOVE A PUBLISHED FIGURE UPWARD: in-force criterion 2 `3/10 = 30.0%` → `5/10 = 50.0%`, all-items `5/15` → `7/15 = 46.7%`, confirmed by a real `run_scoring.run()` BEFORE AND AFTER rather than hand-computed.** `C02-01` gains the coordinated phrase *"Antares or its Subcontractor"* (§3.5.1's rider), `C02-03` gains `invoice_costs`, `C11-01` gains `principal_interest`. **THE v0.58 SIZING WAS RECONFIRMED, NOT CARRIED FORWARD** — it was stated on a `/9` denominator that v0.59's kind-scoping has since moved to `/10`, so the `+2` was re-measured against the committed set; it reproduces exactly, movers `C02-01` and `C02-03`. **`C11-01` is worth ZERO as predicted**, confirmed rather than restated: modal outcome stays `PARTIAL`, failing clauses `['object_class', 'conditions', 'obligee']` → `['conditions', 'obligee']`. **NOT RESTAMPED, and the alternative was MEASURED rather than argued: widen-and-restamp reads `3/7 = 42.9%`** — both movers drop out of the denominator, the numerator does not move at all, and the figure "improves" purely by shrinking, the shape F19 refused. Stamps held at `v0.28`; scoreability unchanged at 15 scored / 21 cassette-unscoreable; no member removed; §3.6.2's restatement screen byte-identical over the widened set; head-only coverage holds at 13/36. **A SECOND-INSTRUMENT EFFECT THE SIZING NEVER PRICED IS DISCLOSED RATHER THAN LEFT TO BE FOUND (§3.6.1's dual-predicate requirement): `K` is UNMOVED at `14/32` and in-force `A` UNMOVED at `7/27`, but the published LEGACY per-clause profile would have drifted `5_object` 6 → 5 of 23 via `C11-01`** — held by the same `PRE_RESTAMP` mechanism F11 and F13 already use, extended to a widening rather than a restamp, so the dated published run still reproduces exactly. **A LATENT DEFECT IN THAT VERY MECHANISM WAS FOUND WHILE EXTENDING IT AND IS FIXED HERE: `PRE_RESTAMP` held `C10-01` TWICE**, so F19's v0.61 `known_gaps` block silently overwrote F11's v0.44 slot correction and Python reported nothing. **It was INERT** — `C10-01` fails `5_object` under either reading, so `K` reproduced correctly throughout and no published figure was ever wrong — **but the guard was dead for two rulings**, and would have failed silently the moment `C10-01`'s own set was widened. Merged into one entry, with an AST-based duplicate-key test that reads the module's source (the evaluated dict is already collapsed) and was verified RED against the reintroduced defect before being trusted. **DONE** | yes | §3.4, §3.6 |
...
| **F18** | `E03-01` today; the `WITHHELD_VALUE` kind generally | **RULED v0.60 — §8.10.1: DIRECTION IS `INCOMPLETENESS`, AND THE KIND STAYS EXCLUDED. THE FIGURE DOES NOT MOVE (`3/10 = 30.0%`), confirmed by a real run before and after; only the REASON changes, from *held* to *ruled*.** **The filed premise is falsified by measurement, and that is what decides it**: the redaction removes the **value** and leaves the **construction**, which the pipeline reads — `E03-005` quotes `"At least ** before the ** of each Calendar Quarter…"` into `temporal_raw` on **3 of 3** runs, and the repaired narrowing classifies as `BEFORE "the ** of each Calendar Quarter"` (verified against the production classifier, not read off the repair text). So gold's `null` does not record an absent constraint; it **discards a stated one**. **Ground 2 bites because §5 reads the PREDICATE, not the item**: §8.1 concedes the conflation in its own words and answers it with `missing_fields` + `redacted_phrase`, both of which are invisible to §5 — so the bare `null` is what a `FULLY_CORRECT` verdict certifies. A **second, weaker** ground is recorded separately (the item is not winnable in practice) and is explicitly **not** `REACHABILITY`'s grammatical guarantee. **AGAINST INTEREST: on this item the model's IR is MORE faithful than gold's** — it recovers the structure and loses only the unknowable magnitude — which is an argument for revisiting §8.1's `null` rule, **not** for admitting the item, and the two are kept apart. The kind is **not** collapsed into `REPRESENTATIONAL` (no IR change helps; v1 *has* the form), and the direction axis is restated on the test that actually does the work: **direction is assignable wherever gold departs from the document**. Rejected alternative measured at `3/11 = 27.3%`. **DONE** *(original entry follows, left as written)* **DOES `redacted_value` CARRY A `GAP_DIRECTION`, AND THEREFORE DOES ITS KIND ENTER §9.1'S DENOMINATOR?** §8.10 rules the kind distinct — not representational (no IR change helps) and not a corpus defect (nothing is wrong with the text; the value is absent from the world the contract describes) — and **deliberately does not rule its direction**. The two-sidedness is real: gold asserts `null` for a field the contract **does** populate, which reads as `INCOMPLETENESS`, yet the loss is not the IR's, which is the whole reason the kind exists. **v0.59 holds it EXCLUDED — the status quo — on the ground that membership cannot be settled on ground 2 while ground 2's own input (is this a knowing unfaithfulness?) is open.** Measured, admitting it reads **`3/11 = 27.3%`** against the ruled `3/10 = 30.0%`. Decide the direction first; membership follows from it | yes (`E03-005`, 3 runs) | v0.59 §8.10 ruling; §8.1, §9.1 |
| **F19** | `E03-01`; any redacted item whose underlying temporal is itself composed | **RULED v0.61 — §8.11: YES, IT OWES A SECOND GAP, AND THE ANNOTATOR'S STATED MECHANISM IS WRONG. `lead_time_unrepresentable` is added (`REPRESENTATIONAL` / `INCOMPLETENESS`); NO PUBLISHED FIGURE MOVES — in-force `3/10 = 30.0%`, all-items `5/15 = 33.3%`, confirmed by a real `run_scoring.run()` before and after.** The *conclusion* survives: through the production `compile_candidate()`, both the phrase as emitted (3/3 runs) and a plausible unredacted one return `UNMAPPABLE_TEMPORAL`. The *mechanism* does not: it is **not** a §7 composition gap. Neither claimed operand is available for this text (`Duration` has no `UNRESOLVED` alias variant, so `each Calendar Quarter` has no form; `RelativeToTriggerTemporal` has no `duration` slot, so `at least ** before` has no form), and the note omits `during the Term`, §7's actual operand. **THE DECISIVE TEST: granting free composition of all five forms does NOT rescue the clause**, because composition joins whole forms and **no form pairs a `duration` with a `direction`** — the nearest composable approximation (`WITHIN 30d OF X` ∧ `BEFORE X`) denotes the **complement** of *"at least 30 days before X"*. So the gap is **single-form expressiveness**, and it earns its own tag rather than a composition one. **NOT RESTAMPED, and the alternative was MEASURED rather than argued**: `known_gaps` is not one of §5's eight scored clauses so this cannot change how the item scores, and restamping would stale `E03-005`'s cassettes and move all-items **`5/15 = 33.3%` → `5/14 = 35.7%`** by dropping a `PARTIAL` item out of the denominator — a figure improving for no improvement. **THE TWO ANNOTATORS' AGREEMENT IS NOT CORROBORATION**: cold also tags `redacted_value` alone, but explicitly declined the composed reading and named a **different** composed element (`DURING`, not `EVERY` ∘ `RELATIVE_TO_TRIGGER`). **A GUIDELINE ERROR IS CORRECTED, AND IT HAD PROPAGATED**: §8's `EVERY`+`DURING` row stated the grammar path (`EVERY`-only-with-warning) where the extraction path is `UNMAPPABLE_TEMPORAL` — the **fourth** tracked instance of a grammar-layer guarantee the extraction path bypasses, and the first cited by an annotator as a reason not to tag. **F17 IS UNMOVED, measured on a scratch copy: the kind-scoped band holds at `(17, 19)`** — `redacted_value` already excludes the item on both sides and membership is by ANY excluding tag — **but `G` moves `6/31 → 7/31`** (Wilson lower 9.19% → 11.40%) as the pair becomes `SUPERSET`. **CORRECTION TO THIS RULING'S OWN FIRST PRICING, made before it landed: F19 is NOT verdict-neutral.** The all-pairs `G` does stay `REDESIGN` either way — but the **conforming-scoped** `G` (§5.1, a second instrument with its own `n`) moves `4/26 → 5/26` and **crosses its `REDESIGN` anchor**, so v0.48's framing that restricting `G` to conforming pairs *"moves it off its own REDESIGN trigger"* is true of the published run and **false of the current set**. No headline figure moves and `G_swing` on conforming pairs stays `0` with the band the point `(15,15)` — the annotators still do not disagree about which conforming items are **scoreable** — but *"no verdict moves"* would have been wrong. **A new entry is opened rather than folded in, F20** (the `G` reproduction's population/tag pinning). **DONE** *(original entry follows, left as written)* **DOES `E03-01` ALSO OWE AN UNTAGGED `REPRESENTATIONAL` GAP, MASKED BY ITS REDACTION TAG?** Surfaced while ruling F18 and deliberately **not** folded into it. The item's own `annotator_notes` record that *"the temporal would fail even unredacted: the underlying obligation is recurring-with-lead-time, plausibly `EVERY` composed with `RELATIVE_TO_TRIGGER`, which IR v1 cannot represent (§7 composition gap)"*. If that reading holds, the item carries **two** gaps — a withheld value **and** a composition gap IR v1 has no form for — and only one is tagged, so `known_gaps` under-reports why the item cannot be scored. **Kept out of the F18 ruling on purpose**: it is a fact about ONE item, and ruling a KIND on a sample of one is the error §22.1 refused. **Exclusion-neutral today** — the item is already excluded on `redacted_value` alone, so adding a tag moves no published figure; what it moves is the ACCURACY of the tag record and, through it, any future per-tag or kind-scoped figure (see **F17**, which is exactly a kind-scoped band). **Not yet verified**: the claim is the original annotator's, recorded as *plausible*, and it needs checking against §7's composition rules and the real IR before any tag is added | yes (`E03-005`; the item's own notes) | v0.60 §8.10.1 (F18) ruling; §8.1, §7 |
...
| **F16** | **whole set — RESOLVED 2026-09-16** | **RESOLVED 2026-09-16, commit `94a2daa`, CI run [35087036051](https://github.com/Arrkkkk/obligo/actions/runs/35087036051) green (562 passed, 5 skipped, 0 failed) against headSha `94a2daa6400bf362c5cb167a784febe16fc73c6a`.** The fix taken is the **first** of the three candidate resolutions this row named and pre-selected none of: **a per-item, not per-run, `guideline_version` comparison.** `cassette.StaleCassette` now carries WHICH dimension(s) mismatched (`.dimensions`); `run_scoring.runs_for_item()` checks each locked item's OWN `guideline_version` stamp against its segment's cassette, so a segment shared by items stamped at different versions (§22.3's validity-transfer case, `C04-117`) correctly scores the matching item and correctly, LOUDLY excludes the mismatched sibling — disclosed by item id and reason (`report.py`'s new G8 section), never a crash. Every OTHER staleness dimension (segment_text, model_id, prompt_version) is checked exactly as before and is still fatal for the whole segment — **§22.1's staleness dimension itself is untouched, nothing is restamped, no cassette is weakened.** A second, previously-latent blocker of the identical shape was found getting the fix to actually complete — R5's self-check and the segment loop both assumed a committed §21 R1-R3 scoring registry for every locked item's document, which batch 3's never-spent documents don't have; `registry_backed_items()` applies the same never-spent-→-disclosed-unscoreable treatment there, not a new judgment call. **Confirmed by a real run, not merely by the fix compiling:** 17/32 locked items disclosed cassette-unscoreable — exactly the 3 already-named known-stale items (`C04-01`, `E01-01`, `C17-02`) plus batch 3's 14 items, no more, no fewer — 15 scored, and **in-force criterion 2 reproduces `3/9 = 33.3%` by a run for the first time since v0.48**, matching the hand-computed figure §9.2 already carried. Real tests added, including planted defects distinguishing a guideline-only `StaleCassette` from a structural one and an overlap-rejection invariant (an item cannot be both scored and disclosed-unscoreable). **No annotation rule changed, no item restamped, no guideline-version bump** — this is a harness/tooling fix, not an annotation decision, so it does not move the guideline's own version number. *Original entry follows, left as written:* **THE SCORER CANNOT RUN END TO END AGAINST THE COMMITTED GOLD SET AT ANY `guideline_version`. This is a broken invariant, not a design question — which is why it is ranked above the five open design entries rather than queued behind them.** `Cassette.verify()` tests `guideline_version` by exact string equality against **one run-level value** (`run_scoring.run()` passes a single `guideline_version` to every `available_runs()` call), and a stale cassette **propagates by design** — `available_runs`' own docstring: *"A stale cassette is not silently treated as a missing run -- it propagates, because replaying it would answer a question that was never asked."* **Verified by execution against the committed artifacts, not inferred:** cassettes stand at `v0.28 ×32` / `v0.48 ×3`, so at **`v0.28`** the run dies on `C04-117` (all 3 cassettes `StaleCassette`) and at **`v0.48`** it dies on **11 segments**; separately, all **10 batch-3 segments have no cassette at all**, which raises `"segment <id> has no usable cassette; cannot score it"`. **Cause, and it is on the record rather than mysterious: the v0.48 freeze-pass batch restamped `C04-02` and `E01-01` ALONE, which is exactly the move §22.1's own "what must NOT happen" list forbids** — *"conforming `C14-02` alone (it yields mixed stamps and the scorer will refuse to run)"*. §9.2 and the v0.49 changelog disclose the resulting mixed-stamp **population** and caveat the `3/9` figure accordingly; **neither states that the entry point itself no longer executes**, so the breakage has been carried as a caveat about a number rather than as a defect in the instrument. **Consequence, stated precisely: every criterion-2 figure since v0.48 has been produced by a scoped or per-segment path, not by `run_scoring.run()` over the committed set, and no session record says which.** That is a reproducibility gap on this phase's own headline acceptance figure. **It also blocks live measurement of both v0.52 rulings' stated costs** — §8.6.1's zero-effect claim and Decision 3's denominator arithmetic are each computed by hand here and cannot currently be confirmed by a run. **NOT a licence to weaken a staleness dimension**: §22.1 rejected exactly that and its reasoning is untouched. The candidate resolutions — a per-item rather than per-run `guideline_version` comparison; an explicit scoped-subset entry point that names which items it scored; or a full §10 conform that restores a single stamp — are a real choice and this row does not pre-select one. **Decide before executing the freeze pass, because the freeze pass is the thing that would otherwise hide it.** | yes (diagnosis is over the cassette directory) | 2026-09-05 F13 / decision-3 ruling session; §22.1, §9.2 |
| **F20** | the `G` instrument's real-data reproduction | **SHOULD A FUTURE §7 RUN SEAL `known_gaps` INTO ITS OWN ARTIFACT, SO OVERRIDE MAPS BECOME UNNECESSARY?** Surfaced while pricing F19's cost against `G` and deliberately **not** folded into it. `comparison.json` seals the population and K's per-clause fails but **NOT** `known_gaps` — `G` did not exist when it was written — so the only way to reproduce a published `G` is a hand-maintained `PRE_TAG` override map, which is a standing maintenance burden and a silent-failure surface of its own (a stale entry rewrites an item that never moved). **The immediate defect is CLOSED, not deferred**: the population is now derived from the sealed artifact, `PRE_TAG` holds the four drifted items' sealed tags (`C04-02`/F7, `C10-01`/F11, `E01-01`/F8, `E03-01`/F19), a genuine historical reproduction sits alongside the live-set one, and tests pin both that they disagree and that no `PRE_TAG` entry has gone inert. **What is open is only the forward design question.** Worth stating because it is the finding that motivated it: the published `G = 6/31` survived the first three drifts **by coincidence** — `C04-02` leaving `swing` and `C10-01` joining `superset` cancelled exactly — so live and sealed agreed on `G` while disagreeing on `G_swing` (1 vs 2) and the band ((16,17) vs (15,17)), and nothing could tell a reproduction from a coincidence. **Bears on F17**, which is a kind-scoped band over this same instrument | yes (measured at seal commit `628f67c`) | v0.61 §8.11 (F19) ruling; §7.1, F17 |
| **F21** | the `temporal_composition` tag; the direction axis generally | **IS `direction` A PROPERTY OF THE TAG OR OF THE ITEM? OPENED v0.63 BY §8.12's RULING, AND DELIBERATELY NOT ANSWERED THERE.** `temporal_composition`'s direction **varies by instance and within a single item**: dropping a NARROWING bound (`during the Term`, `until an alternate source is secured`) leaves the IR claiming MORE than the contract, dropping an ADDITIVE deadline (`thereafter by 31 December of each Calendar Year`) leaves it claiming LESS, and **`C04-08` drops one of each**. Every other direction-bearing tag has been uniform across its members, so the axis has never been asked this. **An OVERSTATING assignment was drafted and REJECTED** — attested 2 of 2 instances, matching `unless_unsupported`'s own carve-out ground, and forced in instance one where gold's narrowing bounds are exactly the unrepresentable ones — because winning on instance count is not describing the class, and a tag-level value would certify a direction that does not describe a specific item (§9.1 ground 2's own failure). **Candidate resolutions, none pre-selected:** (a) keep direction per-tag and accept that a mixed tag simply cannot carry one, leaving `DIRECTION_DEFERRED` permanent for such tags; (b) make direction a per-ITEM field on `known_gaps`, which is a schema change reaching every item and every consumer of G6; (c) split the tag by polarity (`temporal_composition_narrowing` / `_additive`), which is cheap in the vocabulary and pushes the judgment onto the annotator per element — and would have to say what `C04-08`, which is both, carries. **COST TODAY IS ZERO and that is why deferring is available**: §9.1's denominator reads KIND, and `REPRESENTATIONAL` excludes on its own. **OPEN** | not cassette-backed (no item restamped) | v0.63 §8.12 |
| **F22** | `C10-02`, `C14-04`, `E08-04` (logged); the whole 8-item `action_not_in_taxonomy` class | **RULED v0.64 (§8.8.5) — §8.8's SUB-CHOICE IS CONFIRMED AND ITS *"pending confirmation"* FLAG IS DISCHARGED.** The §8.8-vs-§14.3 tension is a **routing error at §14.1**, not a clash of live rules: §14.3's precondition is *sentence* ambiguity, a genuine gap is *encoding* uncertainty (§14.1's `UNCERTAIN` → §14.4), and all three logged items in fact took §14.4 (`adjudicated_by: reviewer`). Independent second ground: §14.3's remedy presupposes each reading is REPRESENTABLE, so widening here would encode indecision among known-wrong answers — §14.2's prohibition at clause 2. **The narrow fix is REJECTED on a measurement**: across 4 of 4 items that recorded a runner-up it admits NOTHING (the runner-up is a competitor to the nearest verb, not a synonym of it; `E03-03`'s genuine synonyms are all off-taxonomy), and it has no mechanical test — §8.3.2's own objection. **The empty-set alternative is RE-PRICED with v0.59's kind-scoping and still rejected**, on the narrower ground that both figures are recoverable from the committed data either way. **Class is EIGHT items (16.7% of 48), not three** — inside §8.8's sampled 23% CI, which is NOT restated. **`E08-04`'s provisional disposition is DISCHARGED, not changed**: no field moves, no restamp (v0.54 precedent). **Added: a G9 report disclosure** naming clause 2 on these items a forced-choice agreement check rather than an action-identification measurement. **No published figure moves, and a widening could not have moved one either** — four independent grounds, each verified by execution. **CLOSED** | not cassette-backed (no item restamped; all 8 are cassette-unscoreable today) | v0.64 §8.8.5 |
| **F23** | the `calendar days` / unit-modifier gap in `_WITHIN_RE`; `E02-02` today | **IS THE UNIT-MODIFIER GAP ITS OWN TAG, OR NOTHING AT ALL? OPENED v0.65 BY §8.13's RULING AND DELIBERATELY NOT ANSWERED THERE.** `_UNIT_ALTERNATION` has no entry for `calendar days`, so it is a THIRD `_WITHIN_RE` reachability defect, mechanistically distinct from the numeral and the preposition (a different sub-pattern, and a trivially different fix — a one-word alternation widening against genuine numeral parsing). **It is not minted, on a measurement: 8 instances across 6 documents (`calendar days` ×7, `continuous months` ×1) and ZERO occur WITHOUT the numeral gap** — there is no `unit` or `unit+preposition` cell in the 179-deadline decomposition at all, so a separate tag could never fire alone and repairing the unit alternation alone would fix ZERO phrases in this corpus. §22.1's discipline applies: a kind is not ruled on a sample of one. **FORCING FUNCTION: a second independent instance, or — far more informative — an instance where the unit gap occurs WITHOUT the numeral gap.** **A SEPARATE, REPORTED-NOT-FIXED DEFECT TRAVELS WITH THIS ROW: `corpus.py`'s `classify_within()` silently buckets all 8 unit instances as parenthetical**, so the published `within_parenthetical` statistic is inflated by 8/159 (5.0%) and §11's still-open `_WITHIN_RE` widening decision is made against that number — §8.6's own objection to folding differently-caused classes together, arriving from the other side. A test pins that no unit tag exists in either vocabulary, so minting one later is a visible decision. **Cost today is ZERO**: `E02-02` files `within_parenthetical` under §8.6's two-gaps-one-tag practice either way, and it is cassette-unscoreable regardless. **OPEN** | not cassette-backed (`E02-006` has no cassette) | v0.65 §8.13 |
| **F24** | `E03-046`'s excluded clause C2 (the ruling itself); §2's cross-reference row and its own v0.28 verbatim-quotation carve-in | **DOES THE CONDITION-TRUTH-VALUE DISCRIMINATOR BECOME A §2 AMENDMENT, OR STAY CASE-LAW? OPENED 2026-09-24 BY `E03-046`'s EXCLUSION AND DELIBERATELY NOT ANSWERED THERE.** **The discriminator is APPLIED AND SETTLED for `E03-046` itself — that half is closed, not queued.** Reviewer-ruled: *does the out-of-corpus reference define a term the condition's **TRUTH VALUE** depends on, or does it merely supply **PROCEDURAL MECHANICS** once the condition is already known to hold?* — truth value → exclude; mechanics only → the verbatim carve-in governs and the item is annotated. **This REFINES `C11-079`'s *"operative content lives outside"* test rather than applying it**: "operative content" is satisfied by almost any substantive reference, including purely procedural ones, so it cannot separate `C11-079` from `C03-01` item 4 without a further judgment it does not supply — which is exactly why those two precedents have sat side by side as *"operative content"* versus *"scope detail"* with **no stated axis between them**. `C11-079` passes the sharper test too, so nothing is overturned. **APPLIED ON A MEASUREMENT, not an assertion: `"cure"`/`"uncured"` occurs EXACTLY ONCE in all 54,541 characters of `E03` — in the excluded sentence itself — and `"Section 13.2.1"` likewise occurs exactly once, inside the reference**, so the condition's entire content sits in an agreement absent from this corpus. **THE CONTRAST PAIR IS A LOCKED ITEM, which is what makes the axis real rather than invented for one segment: `C13-03`** — same construction, same `MAY`, same `TERMINATE`, same `object_class: agreement` — **is annotated**, because its gating event (*"becomes insolvent"*, *"bankruptcy or insolvency proceedings are instituted"*) is fully stated and externally evaluable in-span. **WHAT IS OPEN IS ONLY WHETHER THIS BECOMES A FORMAL §2 AMENDMENT.** It is currently **forward guidance inside the exclusion's own log entry**, on `C14-139`'s established precedent for a well-reasoned ruling with a single instance (§14.4: *repeated* uncertainty is a missing rule; one instance is a logged ruling). **FORCING FUNCTION: a second instance — most informatively one on the KEEPING side**, where a reference inside a verbatim `conditions` entry supplies procedure only and the item is annotated over it. That would give the axis **both** of its cells, which this one instance does not. **Costs nothing to leave open and moves no published figure either way** — an exclusion adds no item, stales no cassette, and touches no scored clause. | no — the segment is excluded whole, so nothing was ever recorded | `evals/goldens/batch05/exclusions.json`, entry `E03-046`, observation `E03-046#condition-truth-value-vs-procedural-mechanics` |
| **F25** | the EVENT-BOUNDED INTERVAL temporal class; `C10-03` today | **IS AN EVENT-BOUNDED INTERVAL ITS OWN TAG? OPENED 2026-09-24 BY `C10-025`'s ANNOTATION AND DELIBERATELY NOT ANSWERED THERE.** *"during the pendency of such challenge"* has **no v1 temporal form at all** — established by reading `ast.py` rather than the regex: **`DuringTemporal` is `{start: DateRef, end: DateRef}`, a DATE PAIR**, and an interval bounded by the pendency of an event has no slot in it at any surface form. Confirmed by execution through the production `_classify_temporal()` (gated on a known accept and a known reject first): `None`. **REPRESENTATIONAL, not REACHABILITY**, on §8.6.1's test — this is not a surface pattern rejecting input the IR could hold, which is what `within_parenthetical`/`within_preposition` are — so `temporal: null` is the null §8.6.1 licenses. **DISTINCT FROM BOTH EXISTING TEMPORAL TAGS, checked against each rather than assumed novel: NOT `temporal_composition`** (§8.12 — a SLOT gap, several elements against one field; here there is exactly **one** element, so it is a FORM gap) **and NOT `lead_time_unrepresentable`** (§8.11 — a duration paired with a direction; here there is **neither**). §8.11 is its nearest sibling, both being single-form expressiveness failures. **NOT MINTED, on §22.1's discipline and F23's direct precedent** — a kind is not ruled on a sample of one — **and deliberately NOT measured pool-wide either**, since a corpus sweep is §10 investigation work that belongs in its own session rather than bundled into an item's drafting. **THE COST RUNS THE OPPOSITE WAY TO MOST UNTAGGED GAPS AND IS PRICED HERE SO IT IS NOT RE-ARGUED: with `known_gaps` EMPTY, `C10-03` ENTERS §9.1's in-force criterion-2 denominator** — an untagged item is a scoreable item — **while carrying a temporal v1 provably cannot represent**, so it is likely a guaranteed failure in the headline denominator (an extractor quoting the phrase into `temporal_raw` gets `UNMAPPABLE_TEMPORAL` and the whole candidate is rejected — §15.3's LOUD side). Latent only because `C10-025` has no cassette. **FORCING FUNCTION: a second independent instance — most informatively one in a CASSETTE-BACKED segment**, which would convert that cost from latent to measured. | no — `C10-025` has no cassette | `evals/goldens/batch05/segments/C10-025.json`, observation `C10-025#event-bounded-interval`; `evals/goldens/batch05/items/C10-03.json` |
| **F26** | `E03-04`/`E03-05` (the ruling itself); §2's agreement-to-agree treatment; `C05-043`/`C04-101` against `C04-02` | **DOES THE AGREEMENT-TO-AGREE DISCRIMINATOR BECOME A §2 AMENDMENT, OR STAY CASE-LAW? OPENED 2026-09-25 BY `E03-022`'s ANNOTATION AND DELIBERATELY NOT ANSWERED THERE.** **The discriminator is APPLIED AND SETTLED for `E03-022` — that half is closed, not queued.** Two committed precedents pointed OPPOSITE WAYS and had **never been distinguished from each other**: `C05-043` + `C04-101` EXCLUDED as agreements-to-agree (*"its content is not readable at segment scope until a separate agreement exists"*, ratified at B4), against **locked item `C04-02`**, an agreement-to-agree with an equally undetermined outcome, ANNOTATED with `action: NEGOTIATE`. Reviewer-ruled: **is the duty ITSELF the act of reaching agreement** (annotatable — `action`, `obligor` and `object_class` are all fully readable from the span even though the eventual agreement's CONTENT is not), **or is some OTHER, SEPARATE duty conditioned on a future agreement existing** (excluded — the qualifier sits on top of a different obligation and makes THAT obligation's own content unreadable, which is `C05-043`'s *"as mutually agreed by the parties"* shape)? **It is sharper than *"is the duty's content readable"*, because BOTH sides have indeterminate outcomes** — which is precisely why the two precedents had coexisted unreconciled. **WHAT IS OPEN IS ONLY WHETHER THIS BECOMES A FORMAL §2 AMENDMENT**; it is currently forward guidance in `E03-04`/`E03-05`'s notes, on `C14-139`'s precedent for a ruling with a single instance. **FORCING FUNCTION: a second instance, most informatively one on the EXCLUDING side**, which would give the axis both cells — the identical shape F24 carries. **THIS IS THE SECOND PREVIOUSLY-UNDRAWN DISTINCTION BETWEEN TWO COMMITTED PRECEDENTS FOUND IN ONE BATCH** (F24 is the first, `C11-079` against `C03-01` item 4), and that pattern is itself worth noticing: both were pairs cited for years as *"operative content"* versus *"scope detail"*, or as exclude-versus-annotate, **with no stated axis between them**. A third occurrence should prompt asking whether §2's exclusion rows generally need their discriminating axes written down rather than inferred per item. | no — `E03-022` has no cassette | `evals/goldens/batch05/items/E03-04.json` and `E03-05.json` (`annotator_notes`); `evals/goldens/batch05/segments/E03-022.json` |
| **F27** | `corpus.split_sentences()`, and therefore every §2.7 per-sentence disposition; `C04-144` today | **`split_sentences()` OVER-SPLITS, AND THE POOL IT FEEDS IS THE KNOWN-ANSWER GATE EVERY SCREEN IN THIS REPOSITORY DEPENDS ON. FOUND 2026-09-26 WHILE DISPOSITIONING `C04-144`; MEASURED, NOT FIXED.** A `[...***...]` confidential-treatment redaction ends in `.` followed by `]`, which the splitter reads as a sentence boundary — so a sentence containing one is returned as two fragments. **MEASURED under a two-sided gate** (known positive `C04-144` → 2 fragments; 3 redaction-bearing segments → 0, proving the detector discriminates rather than firing on any redaction): **32 of the 35 redaction-bearing pool segments split this way — 91.4%, ALL in `C04`.** `C04-144` itself reports 5 fragments where the truth is **3**. **THE DEFECT HAS AT LEAST TWO INDEPENDENT CAUSES, which is why a redaction-scoped fix would be incomplete: `C04-072` ALSO yields a fragment ending `"i.e."`** — an abbreviation break with nothing to do with redaction, found incidentally in the same run. **NO LOCKED ARTIFACT IS WRONG, verified rather than assumed.** Exactly one locked segment is affected — `C04-072`, backing `C04-08` — and the splitter yields **8** fragments there while its committed §2.7 disposition records **`sentences_total: 5`**: the batch-4 annotator silently ignored the over-split. **All seven segments dispositioned in batch 5** (`E02-006`, `E03-037`, `E03-046`, `C04-174`, `C10-025`, `E03-022`, `E01-004`) were checked individually and carry **zero** redaction splits and **zero** abbreviations. So this is a TOOLING hazard, not a data defect. **THE REASON `corpus.py` IS NOT TOUCHED, and it is not caution for its own sake:** `enumerate_pool()` feeds `build_pool()`, whose **1,547-segment count is the known-answer gate** that `§8.8.4`'s `responsible_for_rule.py`, `§8.12`'s `temporal_composition/screen.py`, `§8.13`'s `within_parenthetical/screen.py`, `band_risk/`'s scripts and every corpus measurement in §8 assert before reading any count off themselves. **Changing the splitter moves that number, silently invalidating every published corpus figure at once** — including the 179-deadline decomposition, the 121-sentence `responsible for` census and the 1,547 gate itself. A fix therefore needs its own dedicated session with a **full re-verification of every existing screen**, and must never be bundled into an annotation pass. **A STANDING PRINCIPLE 7 CASE AGAINST THE ANNOTATOR'S OWN METHOD, recorded because that is the reusable half:** this session gated the splitter against locked `E02-006` and reported "GATE PASSED" seven times — but `E02-006` contains no redaction and no abbreviation, so **the gate exercised surface form and could never reach the failure mode**. Principle 6's complaint about generators that vary only surface form, arrived at through a *gate* instead. **FORCING FUNCTION: a segment where the over-split would actually change a §2.7 clause count or a §2 band decision** — `C04-072` did not, because its annotator overrode the tool by hand. | no — `C04-144` has no cassette | `evals/goldens/batch05/segments/C04-144.json` (observation, when that segment is dispositioned); measured by `holdout/` screen re-run 2026-09-26 |
...
by hand — this precedence note stays as written above because it correctly described the state
between 2026-09-05 and 2026-09-16, not because it is still true.
...
- Every such item is tagged `known_gaps: ["within_parenthetical"]` so the two denominators
  ...
  > **CORRECTION IN PLACE (v0.65) — this instruction named a tag that DID NOT EXIST IN CODE for
  > seventeen versions, and no item could have followed it without failing a test.** Verified by
  > execution, not by reading: `within_parenthetical` was absent from `gap_kinds.GAP_KIND` and from
  > `annotator_agreement.SECTION_8_TAGS`, and carried by zero of the 48 locked items — so §8.10's
  > forcing function would have failed the first item that used it, for want of a ruled `kind`.
  > **It is minted at §8.13, kind `REACHABILITY`**, and `E02-02` is its first carrier. The line
  > above is left as written because it was right about what should happen; what was missing was
  > the implementation. **§8.6's "two gaps, one tag" practice is reconciled with this line at
  > §8.13 rather than silently merged** — the two contradicted each other, in the same shape as
  > F13's original conflict, and CLAUDE.md's `C11-03` entry records the premise §8.6 was operating
  > on (*"the numeral gap has no tag"*), which was true of the code and false of this section.
  > **`C11-03` keeps `within_preposition` alone and is not restamped.**
...
**AMENDED v0.64 (§8.8.5, §10.1 F22) — THE FIRST BULLET DOES NOT REACH A §8.8 GENUINE-GAP ITEM,
and the apparent conflict with §8.8's no-widening rule is a ROUTING ERROR AT §14.1 rather than a
clash between two live rules.** §14.1's own definition is *"**The sentence** admits ≥2 defensible
readings"*, glossed *"`AMBIGUOUS` is a property of the text."* Where the taxonomy has no member
denoting the performance, the **text** is not in doubt — `C10-02` says *add*, `E08-04` says
*dispute* — and what is uncertain is the **encoding**, which is §14.1's `UNCERTAIN` and routes to
§14.4. Independently of that reading: **§14.3's remedy presupposes each competing reading is
REPRESENTABLE.** Widening records that the sentence admits both, and in a genuine gap **none** of
the candidates is correct, so a widening would record the annotator's indecision among known-wrong
answers — §14.2's prohibition, applied to clause 2 instead of clause 8. **`action_accept_set`
records what the SENTENCE admits, never what the TAXONOMY forces the annotator to settle for.**
The full ruling, the eight-item census and the rejected narrow fix are §8.8.5.
...
> **CORRECTION IN PLACE (v0.65): THE RULE STANDS; ITS CITED EVIDENCE DOES NOT SURVIVE CHECKING.
> ALL THREE of the collisions the paragraph above names were measured against the corpus and the
> committed registries, and NONE of them reproduces.** Found while authoring `registry/E02.json`
> for batch 5's first E02 item — the first time any session had reason to read E02's preamble.
>
> | cited collision | what the documents actually define |
> | :--- | :--- |
> | `Client` in **E02** and E07 | E07 defines `Client` (= Instinet Corporation). **E02 defines no bare `Client` at all** — all 22 occurrences of the token are inside `Client Facility`, which E02 defines as *"the departments and facilities of the CBay customer"*, an **unnamed third party R3 excludes**. E02's registry could never have held the entry |
> | `Provider`/`Recipient` in C17 and **E01** | C17 defines both as roles and its registry carries them. **E01 defines `Service Provider` / `Service Recipient`** — 78 and 57 occurrences, **zero** bare `Provider`/`Recipient`, checked for definitional quotation marks as well as raw counts |
>
> **And the stated CONSEQUENCE was tested by execution rather than argued.** A single registry
> merged from all ten committed ones resolves `Client` → Instinct Corporation, `Provider` →
> Provider and `Recipient` → Recipient, **each uniquely** — so `E07-01` and `C17-01` would **not**
> flip, and the specific failure this paragraph predicts does not occur.
>
> **R1 IS NOT AMENDED AND EVERY REGISTRY IN THE REPOSITORY FOLLOWS IT.** Per-document registries
> remain correct on the ground that survives: `resolve_party()`'s `len(rows) != 1` rule is real,
> and `Provider` against E01's `Service Provider` is a genuine **near-miss** that a different
> document, or a registry authored with E01's role terms in it, could turn into an actual
> collision. What is falsified is the **measurement** offered in support, not the rule — the same
> footing as §8.9's `on`/`until` rows and §2.4's *"6 segments, contained"* claim, and the same
> corrections-are-new-text discipline. **Scoped honestly: this was measured over the ELEVEN
> committed registries, not over all 28 documents**, so it establishes that the cited collisions
> do not reproduce, never that no collision exists anywhere in the corpus.
>
> The identical claim is repeated in `evals/harness/registry.py`'s docstring — a **second copy**,
> the same hand-maintained-duplicate hazard v0.61 recorded for the tag vocabulary — and in
> CLAUDE.md's debt list. Both are corrected alongside this one.
