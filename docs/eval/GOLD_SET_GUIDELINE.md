...
| :--- | :--- | :--- | :--- | :--- |
| **`REPRESENTATIONAL`** | IR v1 has **no form** for what the document says; the loss is baked into the gold item itself | **required** | **no** — knowingly unfaithful | `exception_unsupported`, `unless_unsupported`, `compound_action`, `mutual_obligation`, `action_not_in_taxonomy`, `lead_time_unrepresentable` (§8.11, v0.61) |
...
v0.59 this said `REPRESENTATIONAL` alone), state its **direction** as well. An unruled tag is `UNCLASSIFIED`, is
